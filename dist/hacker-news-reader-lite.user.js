...
`)}))}}#c(){if(this.#i||this.#n!==null)return;this.dispatchEvent(new Event("error"));let e=Math.min(3e4,1e3*2**Math.min(this.#r,5));this.#r+=1,this.#n=this.document.defaultView?.setTimeout(()=>{this.#n=null,this.#s()},e)??null}},Ae=class i{constructor(t){this.sourceFactory=t}static fromDocument(t){return new i(typeof GM_xmlhttpRequest=="function"?e=>new En(t,e):void 0)}subscribe(t,e){if(!this.sourceFactory||t.destroyed)return!1;let n;try{n=this.sourceFactory(oi)}catch(s){return e.onUnavailable?.(s instanceof Error?s.message:"HN 实时连接创建失败"),!1}t.add(()=>n.close()),t.listen(n,"open",()=>e.onConnected?.()),t.listen(n,"error",()=>e.onReconnecting?.());let r=s=>{try{let a=ii(s);a.length>0&&e.onItemsChanged(a)}catch{}};t.listen(n,"put",r),t.listen(n,"patch",r);let o=()=>{e.onUnavailable?.("HN 实时流已被服务端取消"),n.close()};return t.listen(n,"cancel",o),t.listen(n,"auth_revoked",o),!0}};function Cn(i){if(!i)return null;let t=/\d+/.exec(i);return t?Number.parseInt(t[0],10):null}function ai(i){let t=i.querySelector(".age")?.title;if(!t)return null;let e=Date.parse(t);return Number.isFinite(e)?e:null}function Nr(i){return i.nextElementSibling?.querySelector(".subtext")??null}function Or(i,t,e,n,r,o){let s=t.querySelector(".titleline > a"),a=s?.textContent?.trim();if(!a)throw new Error("HN story title was not found");let l=Nr(t),d=i.querySelector(".toptext"),h=et(d?.innerHTML??"",i,i.baseURI);return{id:e,title:a,url:s?.href&&!s.href.startsWith("https://news.ycombinator.com/item")?s.href:null,author:l?.querySelector(".hnuser")?.textContent?.trim()||null,score:Cn(l?.querySelector(".score")?.textContent),html:h,childIds:n,descendants:o,observedAt:r}}function In(i,t,e=Date.now()){let n=i.querySelector(`tr.athing:not(.comtr)[id="${t}"]`);if(!n)return null;let r=Nr(n),o=Cn(r?.querySelector(".comments")?.textContent);return{schemaVersion:1,story:Or(i,n,t,Object.freeze([]),e,o),comments:Object.freeze([]),loadedIds:Object.freeze([]),missingIds:Object.freeze([]),complete:!1,capturedAt:e}}function Bt(i,t=Date.now(),e=Number.POSITIVE_INFINITY){if(!(e===Number.POSITIVE_INFINITY||Number.isSafeInteger(e)&&e>0))throw new RangeError("comment limit must be a positive integer");let n=i.querySelector("tr.athing:not(.comtr)");if(!n?.id)throw new Error("HN story row was not found");let r=mt(n.id),o=[],s=[],a=new Map,l=i.querySelectorAll("tr.athing.comtr"),d=0;for(let y of l){if(o.length>=e)break;if(d+=1,!y.id)continue;let f;try{f=st(y.id)}catch{continue}let E=Cn(y.querySelector("td.ind img")?.getAttribute("width"))??0,I=Math.max(0,Math.round(E/40)),T=I>0?s[I-1]??r:r;s.length=I,s[I]=f;let g=a.get(T)??0;a.set(T,g+1);let A=y.querySelector(".commtext"),H=A?.innerHTML??"",C=(A?.textContent??"").replace(/\s+/g," ").trim(),b=y.querySelector(".hnuser")?.textContent?.trim()||null;o.push({id:f,storyId:r,parentId:T,childIds:[],rank:g,author:b,createdAt:ai(y),html:H,text:C,deleted:b===null&&/^\[deleted\]$/i.test(C),dead:y.classList.contains("dead")||A?.classList.contains("cdd")===!0,source:"dom",observedAt:t})}let h=new Map;for(let y of o){let f=h.get(y.parentId)??[];f.push(y.id),h.set(y.parentId,f)}let m=o.map(y=>({...y,childIds:Object.freeze(h.get(y.id)??[])})),p=Object.freeze(h.get(r)??[]);return{schemaVersion:1,story:Or(i,n,r,p,t,l.length),comments:Object.freeze(m),loadedIds:Object.freeze(m.map(y=>y.id)),missingIds:Object.freeze([]),complete:d===l.length&&i.querySelector("a.morelink")===null,capturedAt:t}}var Re=class{constructor(t,e,n){this.hostDocument=t;this.scheduler=e;this.fetchPage=n}load(t,e){let n=`https://news.ycombinator.com/item?id=${t}`;return this.scheduler.schedule({key:`hn-page:${t}`,lane:"hn-interactive",...e?{signal:e}:{},run:async r=>{let o=this.hostDocument.defaultView,s=this.fetchPage??(o?.fetch?o.fetch.bind(o):void 0);if(!s)throw new Error("当前浏览器不支持同源评论页请求");let a;try{a=await s(n,{method:"GET",credentials:"omit",cache:"no-store",redirect:"follow",headers:{Accept:"text/html,application/xhtml+xml"},signal:r})}catch(p){throw r.aborted?r.reason instanceof Error?r.reason:new q("request aborted","aborted"):new q(p instanceof Error?p.message:"network request failed","network")}if(!a.ok)throw new q(`HTTP ${a.status}`,"http",a.status);let l=new URL(a.url||n,n);if(l.hostname!=="news.ycombinator.com"||l.pathname!=="/item")throw new Error("HN 评论页重定向到了非预期地址");if(l.searchParams.get("id")!==String(t))throw new Error("HN 评论页与所选故事不一致");let d=o?.DOMParser;if(!d)throw new Error("当前浏览器不支持 HTML 解析");let h=new d().parseFromString(await a.text(),"text/html"),m=h.createElement("base");return m.href=l.href,h.head.prepend(m),{document:h,finalUrl:l.href}}})}};var li="hnr-avatar-v1:";var Fr="http://www.w3.org/2000/svg",di=new Set(["line","path","polygon","rect","svg"]),ci=new Set(["d","height","points","ry","style","transform","viewBox","width","x","x1","x2","xmlns","y","y1","y2"]),hi=new Set(["fill","opacity","stroke","stroke-linecap","stroke-linejoin","stroke-width"]),Wt=new Map;function Dr(){if(typeof multiavatar=="function")return multiavatar;let i=globalThis.multiavatar;return typeof i=="function"?i:null}function mi(i){let t=i?.trim().toLowerCase()??"";return t.length>0?t:null}function ui(i,t){if(Wt.set(i,t),Wt.size<=256)return;let e=Wt.keys().next();e.done||Wt.delete(e.value)}function pi(i){let t=mi(i),e=Dr();if(!t||!e)return null;let n=`${li}${t}`,r=Wt.get(n);if(r)return r;try{let o=e(n);return!o.startsWith("<svg")||!o.endsWith("</svg>")?null:(ui(n,o),o)}catch{return null}}function fi(i){let t=i.getAttribute("style");return t?t.split(";").every(e=>{let n=e.trim();if(!n)return!0;let r=n.indexOf(":");if(r<=0)return!1;let o=n.slice(0,r).trim().toLowerCase(),s=n.slice(r+1).trim();return hi.has(o)&&s.length>0&&!/(?:url\s*\(|expression\s*\(|@import|javascript:|data:)/i.test(s)}):!0}function yi(i){return i.namespaceURI!==Fr||i.localName!=="svg"?!1:[i,...i.querySelectorAll("*")].every(t=>t.namespaceURI===Fr&&di.has(t.localName)&&[...t.attributes].every(e=>ci.has(e.name))&&fi(t))}function _r(i,t){let e=pi(t),n=i.defaultView?.DOMParser;if(!e||!n)return null;let r=new n().parseFromString(e,"image/svg+xml");if(!yi(r.documentElement))return null;let o=i.importNode(r.documentElement,!0);return o.classList.add("hnr-author-avatar"),o.setAttribute("width","32"),o.setAttribute("height","32"),o.setAttribute("aria-hidden","true"),o.setAttribute("focusable","false"),o}function $r(){let i=Dr();if(!i)return null;let t=Function.prototype.toString.call(i);return!t.startsWith("function multiavatar(")||!t.includes("</svg>")?null:t.replace(/<\/script/gi,"<\\/script")}function at(i){let t={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"};return i.replace(/[&<>"']/g,e=>t[e]??e)}function gi(i){return JSON.stringify(i).replace(/[<>&\u2028\u2029]/g,t=>t==="<"?"\\u003c":t===">"?"\\u003e":t==="&"?"\\u0026":t==="\u2028"?"\\u2028":"\\u2029")}function qr(i){return i.replace(/<\/style/gi,"<\\/style")}function bi(i){let t=rt(i.titleFontFamily,i.titleCustomFontFamily),e=rt(i.fontFamily,i.customFontFamily);return`:host{--hnr-title-font-family:${t};--hnr-content-font-family:${e};--hnr-font-scale:${i.fontScale}}`}function wi(i){let t="";for(let n of i.normalize("NFKC"))t+=n.charCodeAt(0)<32||'\\/:*?"<>|'.includes(n)?" ":n;return(t.replace(/\s+/g," ").trim()||"hacker-news-thread").slice(0,80)}var vi="html,body,#hn-reader-root{width:100%;height:100%;margin:0;overflow:hidden}",Ti=String.raw`
...
${qr(l)}
${Ti}</style><section class="hnr-shell hnr-offline-shell" role="region" aria-labelledby="hnr-reader-title"><header class="hnr-header"><div class="hnr-identity"><div class="hnr-eyebrow">HN READER</div><h1 id="hnr-reader-title" class="hnr-title"><span class="hnr-title-original">${o}</span>${s?`<span class="hnr-title-subtitle" lang="zh-CN">${s}</span>`:""}</h1></div><div class="hnr-offline-toolbar" role="search" aria-label="离线评论工具"><input id="search" type="search" placeholder="搜索评论" aria-label="搜索评论"><select id="mode" aria-label="翻译显示"><option value="original">仅原文</option><option value="bilingual">双语</option><option value="translated">仅译文</option></select><button id="collapse" type="button">全部收起</button><button id="expand" type="button">全部展开</button></div><span class="hnr-coverage">${i.snapshot.comments.length} 条评论 · ${i.snapshot.complete?"完整":"当前快照"}</span><div class="hnr-header-actions"><span class="hnr-offline-badge">OFFLINE</span>${d}<a class="hnr-original-control" href="${at(a)}" target="_blank" rel="noopener noreferrer" aria-label="回到原帖" aria-describedby="hnr-tooltip-offline-original"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"></path></svg><span id="hnr-tooltip-offline-original" class="hnr-tooltip" role="tooltip">回到原帖</span></a></div></header>${h}<div class="hnr-comments" role="tree" aria-label="Hacker News 评论树"><div id="thread" class="hnr-virtual-items hnr-offline-items"></div>${r}</div><footer class="hnr-footer">离线副本 · 可搜索、折叠并切换原文与译文 · 不含 API Key、Cookie 或登录状态</footer></section></template></div><script id="hnr-data" type="application/json">${gi(e)}<\/script><script nonce="hnr-offline-v1">${p}<\/script></body></html>`}function Ur(i){return`${wi(i.snapshot.story.title)}-hn-${i.snapshot.story.id}.html`}function An(i,t,e){let n=URL.createObjectURL(new Blob([t],{type:"text/html;charset=utf-8"})),r=i.createElement("a");r.href=n,r.download=e,r.hidden=!0,i.body.append(r),r.click(),r.remove(),i.defaultView?.setTimeout(()=>URL.revokeObjectURL(n),1e3)}var Rn="offline-history:all",xe="offline-html:",xn=20,He=class{constructor(t,e=Date.now){this.cache=t;this.now=e}async list(){let t=await this.cache.get(Rn);if(t?.kind!=="offline-history")return Object.freeze([]);let e=this.now()-2592e6;return Object.freeze(t.entries.filter(n=>n.savedAt>e).sort((n,r)=>r.savedAt-n.savedAt).slice(0,xn))}async save(t){let e=`${t.storyId}:${U(t.html)}`,n=Object.freeze({id:e,storyId:t.storyId,storyTitle:t.storyTitle,filename:t.filename,savedAt:this.now(),bytes:new TextEncoder().encode(t.html).byteLength,commentCount:t.commentCount,translatedCount:t.translatedCount});await this.cache.set(`${xe}${e}`,Object.freeze({kind:"offline-html",id:e,html:t.html}));let r=await this.list(),o=[n,...r.filter(a=>a.id!==e)],s=Object.freeze(o.slice(0,xn));return await this.cache.set(Rn,Object.freeze({kind:"offline-history",entries:s})),await Promise.allSettled(o.slice(xn).map(a=>this.cache.delete(`${xe}${a.id}`))),n}async getHtml(t){let e=await this.cache.get(`${xe}${t}`);return e?.kind==="offline-html"&&e.id===t?e.html:void 0}async delete(t){let e=await this.list();if(!e.some(r=>r.id===t))return!1;let n=Object.freeze(e.filter(r=>r.id!==t));return await Promise.all([this.cache.set(Rn,Object.freeze({kind:"offline-history",entries:n})),this.cache.delete(`${xe}${t}`)]),!0}clear(){return this.cache.clear()}};function c(i,t,e="",n){let r=i.createElement(t);return r.className=e,n!==void 0&&(r.textContent=n),r}function Si(i){let t=/Firefox\//u.test(i.userAgent),e=/AppleWebKit\//u.test(i.userAgent)&&!/(?:Chrome|Chromium|Edg|OPR|CriOS|FxiOS)\//u.test(i.userAgent);return Object.freeze({stroke:t?.03:e?.05:.015,shadow:t?.55:e?.45:.75,macSmoothing:/Mac/u.test(i.platform)})}function Vr(i,t,e){let n=Si(e);i.dataset.fontRendering=t?"builtin":"off",i.style.setProperty("--hnr-font-rendering-stroke-runtime",`${n.stroke}px currentColor`),i.style.setProperty("--hnr-font-rendering-shadow-runtime",`0 0 ${n.shadow}px #7c7c7cdd`),i.toggleAttribute("data-font-mac-smoothing",n.macSmoothing)}var Le=class{#t;#e;#n;#r=[];#o=[0];#i=0;constructor(t={}){if(this.#t=t.defaultHeight??112,this.#e=t.overscanPx??960,this.#n=t.maxMounted??120,this.#t<=0||this.#e<0||this.#n<1)throw new RangeError("invalid virtual layout options")}get count(){return this.#r.length}setCount(t){if(!Number.isSafeInteger(t)||t<0)throw new RangeError("count must be a non-negative integer");t!==this.#r.length&&(t>this.#r.length?this.#r.push(...Array.from({length:t-this.#r.length},()=>this.#t)):this.#r.length=t,this.#i=Math.min(this.#i,t),this.#o.length=Math.min(this.#o.length,t+1))}updateHeight(t,e){if(t<0||t>=this.#r.length)return!1;let n=Math.max(24,Math.ceil(e));return this.#r[t]===n?!1:(this.#r[t]=n,this.#i=Math.min(this.#i,t),!0)}seedHeights(t){for(let[e,n]of t.entries())this.updateHeight(e,n)}range(t,e){this.#s();let n=this.#o.at(-1)??0,r=Math.max(1,e),o=Math.max(this.#e,r),s=Math.max(0,t),a=this.#l(s),l=Math.min(this.count,this.#l(Math.max(s,s+r-.01))+1),d=Math.max(0,t-o),h=Math.max(d,t+r+o),m=this.#l(d),p=Math.min(this.count,this.#l(h)+1);return p-m>this.#n&&(p=m+this.#n),{start:m,end:p,viewportStart:a,viewportEnd:l,topSpacer:this.#o[m]??0,bottomSpacer:Math.max(0,n-(this.#o[p]??n)),totalHeight:n}}offsetFor(t){return this.#s(),this.#o[Math.max(0,Math.min(t,this.count))]??0}anchorAt(t){if(this.count===0)return null;this.#s();let e=Math.max(0,t),n=this.#l(e);return{index:n,offset:e-(this.#o[n]??0)}}#s(){let t=Math.max(0,Math.min(this.#i,this.#r.length));this.#o.length<t+1&&(this.#o.length=t+1),t===0&&(this.#o[0]=0);for(let e=t;e<this.#r.length;e+=1)this.#o[e+1]=(this.#o[e]??0)+(this.#r[e]??this.#t);this.#o.length=this.#r.length+1,this.#i=this.#r.length}#l(t){if(this.count===0)return 0;let e=0,n=this.count;for(;e<n;){let r=Math.floor((e+n)/2);(this.#o[r+1]??0)<=t?e=r+1:n=r}return Math.min(e,this.count-1)}};var Me=class{constructor(t,e,n,r=new Le,o){this.container=t;this.renderEntry=e;this.onRenderedEntries=o;this.#t=Et.ownedBy(n),this.#e=r,this.#n=t.ownerDocument.createElement("div"),this.#r=t.ownerDocument.createElement("div"),this.#o=t.ownerDocument.createElement("div"),this.#n.className="hnr-virtual-spacer",this.#r.className="hnr-virtual-items",this.#o.className="hnr-virtual-spacer",t.replaceChildren(this.#n,this.#r,this.#o),this.#t.listen(t,"scroll",()=>{let a=this.#c!==null&&Math.abs(t.scrollTop-this.#c)<=1;this.#c=null,this.schedule(!a)});let s=t.ownerDocument.defaultView?.ResizeObserver;this.#i=s?new s(a=>{let l=this.#e.anchorAt(t.scrollTop),d=!1;for(let h of a){let m=Number.parseInt(h.target.dataset.virtualIndex??"",10);Number.isSafeInteger(m)&&(d=this.#e.updateHeight(m,h.contentRect.height)||d)}d&&(this.#h(l),this.schedule())}):null,this.#i&&this.#t.add(()=>this.#i?.disconnect()),this.#t.add(()=>{this.#l!==null&&this.#m(this.#l),this.#l=null,t.replaceChildren()})}#t;#e;#n;#r;#o;#i;#s=[];#l=null;#a=!1;#c=null;get mountedCount(){return this.#r.childElementCount}capturePosition(){let t=this.#e.anchorAt(this.container.scrollTop),e=t?this.#s[t.index]:void 0;return t&&e?{id:e.id,offset:t.offset}:null}restorePosition(t){let e=this.#s.findIndex(r=>r.id===t.id);if(e<0)return!1;let n=this.#e.offsetFor(e)+Math.max(0,t.offset);return this.#c=n,this.container.scrollTop=n,this.refreshNow(!0),!0}setEntries(t,e){let n=this.#e.anchorAt(this.container.scrollTop),r=n?this.#s[n.index]:void 0;if(this.#s=t,this.#e.setCount(t.length),e&&this.#e.seedHeights(e),n&&r){let o=t.findIndex(s=>s.kind===r.kind&&s.id===r.id);this.#h(o>=0?{index:o,offset:n.offset}:n)}this.refreshNow(!0)}scrollToIndex(t){this.#c=null,this.container.scrollTop=this.#e.offsetFor(t),this.refreshNow(!0)}seedHeights(t){let e=this.#e.anchorAt(this.container.scrollTop);this.#e.seedHeights(t),this.#h(e),this.refreshNow()}schedule(t=!1){this.#a||=t,this.#l===null&&(this.#l=this.#u(()=>{this.#l=null;let e=this.#a;this.#a=!1,this.refreshNow(e)}))}refreshNow(t=!1){let e=this.#e.range(this.container.scrollTop,this.container.clientHeight||720),n=this.container.ownerDocument.createDocumentFragment(),r=[];this.#i?.disconnect();for(let o=e.start;o<e.end;o+=1){let s=this.#s[o];if(!s)continue;let a=this.renderEntry(s,o);r.push(s),a.dataset.virtualIndex=String(o),n.append(a),this.#i?.observe(a)}this.#n.style.height=`${e.topSpacer}px`,this.#o.style.height=`${e.bottomSpacer}px`,this.#r.replaceChildren(n),this.onRenderedEntries?.(Object.freeze(r),Object.freeze(this.#s.slice(e.viewportStart,e.viewportEnd)),t)}destroy(){this.#t.destroy()}#u(t){return this.container.ownerDocument.defaultView?.requestAnimationFrame(t)??window.setTimeout(()=>t(performance.now()),16)}#m(t){let e=this.container.ownerDocument.defaultView;e?.cancelAnimationFrame?e.cancelAnimationFrame(t):clearTimeout(t)}#h(t){if(!t)return;let e=this.#e.offsetFor(t.index)+t.offset;Math.abs(this.container.scrollTop-e)>.5&&(this.#c=e,this.container.scrollTop=e)}};var Ei=Object.freeze({"alibaba puhuiti":"阿里巴巴普惠体",dengxian:"等线",fangsong:"仿宋","harmonyos sans sc":"鸿蒙黑体","heiti sc":"黑体-简","heiti tc":"黑体-繁","hiragino sans gb":"冬青黑体简体中文",kaiti:"楷体","kaiti sc":"楷体-简","kaiti tc":"楷体-繁","lxgw wenkai":"霞鹜文楷","microsoft jhenghei":"微软正黑体","microsoft jhenghei ui":"微软正黑体 UI","microsoft yahei":"微软雅黑","microsoft yahei ui":"微软雅黑 UI","noto sans cjk sc":"思源黑体","noto sans cjk tc":"思源黑体繁体","noto serif cjk sc":"思源宋体","noto serif cjk tc":"思源宋体繁体",nsimsun:"新宋体","pingfang hk":"苹方-港","pingfang sc":"苹方-简","pingfang tc":"苹方-繁",simfang:"仿宋",simhei:"黑体",simkai:"楷体",simsun:"宋体","smiley sans":"得意黑","songti sc":"宋体-简","songti tc":"宋体-繁","source han sans sc":"思源黑体","source han sans tc":"思源黑体繁体","source han serif sc":"思源宋体","source han serif tc":"思源宋体繁体",stfangsong:"华文仿宋",stheiti:"华文黑体",stkaiti:"华文楷体",stsong:"华文宋体","wenquanyi micro hei":"文泉驿微米黑","wenquanyi zen hei":"文泉驿正黑"}),Ci=new RegExp("(?:\\p{Script=Han}|cjk|source han|yahei|jhenghei|simsun|simhei|simkai|simfang|dengxian|pingfang|songti|heiti|kaiti|wenquanyi|wenkai|puhui|harmonyos)","iu"),Ii=new Intl.Collator("zh-CN",{numeric:!0,sensitivity:"base"}),Ai=0;function Hn(i){return i.normalize("NFKC").toLocaleLowerCase("zh-CN").replace(/[\s()（）._-]+/gu," ").trim()}function Pe(i){let t=new Map;for(let n of i){let r=n.replace(/\s+/gu," ").trim();if(!r)continue;let o=r.toLocaleLowerCase("en-US");t.has(o)||t.set(o,r)}let e=[...t.values()].map(n=>{let r=Ei[n.toLocaleLowerCase("en-US")],o=r?`${r}（${n}）`:n;return Object.freeze({family:n,label:o,searchText:Hn(`${o} ${n}`),chinesePreferred:!!r||Ci.test(n)})});return e.sort((n,r)=>Number(r.chinesePreferred)-Number(n.chinesePreferred)||Ii.compare(n.label,r.label)),Object.freeze(e)}function Br(i){let t=i.defaultView,e;try{e=t?.queryLocalFonts}catch{return}if(!t||typeof e!="function")return;let n=null,r=null;return async()=>{if(n)return n;if(r)return r;r=Promise.resolve(Reflect.apply(e,t,[])).then(o=>Object.freeze(Pe(o.map(s=>s.family??"")).map(s=>s.family))).then(o=>(n=o,o));try{return await r}finally{r=null}}}var Gt=class{element;fontFamilyInput;customFontFamilyInput;#t;#e;#n;#r;#o;#i;#s;#l;#a=Object.freeze([]);#c=!1;#u=!1;#m=0;constructor(t){this.#t=t.document,this.#e=t.queryLocalFonts,this.element=c(t.document,"span","hnr-local-font-picker"),this.fontFamilyInput=c(t.document,"input"),this.fontFamilyInput.type="hidden",this.fontFamilyInput.name=t.fontFamilyName??"fontFamily",this.fontFamilyInput.value=t.fontFamily,this.customFontFamilyInput=c(t.document,"input"),this.customFontFamilyInput.type="hidden",this.customFontFamilyInput.name=t.customFontFamilyName??"customFontFamily",this.customFontFamilyInput.value=t.customFontFamily,this.#n=c(t.document,"button","hnr-local-font-trigger"),this.#n.type="button",this.#n.setAttribute("aria-haspopup","listbox"),this.#n.setAttribute("aria-expanded","false"),this.#r=c(t.document,"span","hnr-local-font-selected");let e=c(t.document,"span","hnr-local-font-chevron","⌄");e.setAttribute("aria-hidden","true"),this.#n.append(this.#r,e),this.#o=c(t.document,"span","hnr-local-font-popover"),this.#o.hidden=!0;let n=c(t.document,"label","hnr-local-font-search");this.#i=c(t.document,"input"),this.#i.type="search",this.#i.placeholder="搜索本机字体…",this.#i.autocomplete="off",this.#i.setAttribute("aria-label","搜索本机字体"),this.#i.setAttribute("role","combobox"),this.#i.setAttribute("aria-autocomplete","list"),this.#i.setAttribute("aria-expanded","true"),n.append(this.#i),this.#s=c(t.document,"span","hnr-local-font-list"),this.#s.id=`hnr-local-font-list-${++Ai}`,this.#s.setAttribute("role","listbox"),this.#s.setAttribute("aria-label","可用字体"),this.#i.setAttribute("aria-controls",this.#s.id),this.#l=c(t.document,"span","hnr-local-font-status"),this.#l.setAttribute("role","status"),this.#l.setAttribute("aria-live","polite"),this.#o.append(n,this.#s),this.element.append(this.fontFamilyInput,this.customFontFamilyInput,this.#n,this.#o,this.#l),this.#n.addEventListener("click",()=>{this.#o.hidden?this.open():this.close()}),this.#i.addEventListener("input",()=>this.#f()),this.#i.addEventListener("keydown",r=>{r.key==="ArrowDown"&&(this.#M()[0]?.focus(),r.preventDefault())}),this.#s.addEventListener("keydown",r=>{if(r.key!=="ArrowDown"&&r.key!=="ArrowUp")return;let o=this.#M(),s=o.indexOf(this.#t.activeElement),a=r.key==="ArrowDown"?1:-1;o[(s+a+o.length)%o.length]?.focus(),r.preventDefault()}),this.#f(),this.#y(),this.#v(this.#e?"进入字体设置后自动读取浏览器可用字体。":"当前浏览器未开放本机字体列表；仍可搜索后手动使用字体名称。")}get expanded(){return!this.#o.hidden}activate(){this.#u||this.#c||!this.#e||this.#w()}open(){this.#o.hidden=!1,this.#n.setAttribute("aria-expanded","true"),this.#i.value="",this.#f(),this.#i.focus()}close(t=!1){this.#o.hidden=!0,this.#n.setAttribute("aria-expanded","false"),t&&this.#n.focus()}handleEscape(){return this.expanded?(this.close(!0),!0):!1}containsEvent(t){return t.composedPath().includes(this.element)}destroy(){this.#m+=1,this.close()}#h(){return le.filter(t=>t!=="custom").map(t=>({key:`preset:${t}`,fontFamily:t,customFontFamily:"",label:It[t],searchText:Hn(`${It[t]} ${t}`),source:"preset"}))}#b(){let t=this.#a.map(n=>({key:`local:${n.family}`,fontFamily:"custom",customFontFamily:n.family,label:n.label,searchText:n.searchText,source:"local"})),e=this.customFontFamilyInput.value.trim();if(this.fontFamilyInput.value==="custom"&&e&&!t.some(n=>n.customFontFamily===e)){let n=Pe([e])[0];n&&t.unshift({key:`local:${e}`,fontFamily:"custom",customFontFamily:e,label:n.label,searchText:n.searchText,source:"local"})}return t}#f(){let t=Hn(this.#i.value),e=this.#h().filter(r=>!t||r.searchText.includes(t)),n=this.#b().filter(r=>!t||r.searchText.includes(t));if(this.#s.replaceChildren(),this.#p("预设字体",e),this.#p(this.#u?`本机字体 · ${this.#a.length}`:"本机字体",n),e.length+n.length===0&&t){let r=this.#i.value.replace(/\s+/gu," ").trim().slice(0,64);if(r){let o={key:`manual:${r}`,fontFamily:"custom",customFontFamily:r,label:`使用“${r}”`,searchText:t,source:"manual"};this.#p("手动字体名称",[o])}}}#p(t,e){if(e.length===0)return;let n=c(this.#t,"span","hnr-local-font-group");n.setAttribute("role","group"),n.setAttribute("aria-label",t),n.append(c(this.#t,"span","hnr-local-font-group-label",t));for(let r of e){let o=c(this.#t,"button","hnr-local-font-option");o.type="button",o.dataset.fontKey=r.key,o.dataset.fontFamily=r.fontFamily,o.dataset.fontName=r.customFontFamily,o.dataset.fontSource=r.source,o.setAttribute("role","option");let s=r.fontFamily===this.fontFamilyInput.value&&(r.fontFamily!=="custom"||r.customFontFamily===this.customFontFamilyInput.value);o.setAttribute("aria-selected",String(s));let a=c(this.#t,"span","hnr-local-font-option-label",r.label),l=c(this.#t,"span","hnr-local-font-option-sample","中文预览 · Aa 0123");l.lang="zh-CN",l.style.fontFamily=rt(r.fontFamily,r.customFontFamily),o.append(a,l),o.addEventListener("click",()=>this.#A(r)),n.append(o)}this.#s.append(n)}#A(t){this.fontFamilyInput.value=t.fontFamily,this.customFontFamilyInput.value=t.customFontFamily,this.#y(),this.close(!0);let e=this.#t.defaultView?.Event??Event;this.fontFamilyInput.dispatchEvent(new e("change",{bubbles:!0}))}#y(){let t=this.fontFamilyInput.value;if(t!=="custom"){this.#r.textContent=It[t]??It.system,this.#r.style.fontFamily=rt(t);return}let e=this.customFontFamilyInput.value.trim(),n=Pe([e])[0];this.#r.textContent=n?.label??It.custom,this.#r.style.fontFamily=rt("custom",e)}#M(){return[...this.#s.querySelectorAll(".hnr-local-font-option")]}async#w(){if(!this.#e||this.#c)return;let t=++this.#m;this.#c=!0,this.#v("正在请求浏览器本机字体权限…");try{if(this.#a=Pe(await this.#e()),t!==this.#m)return;this.#u=!0,this.#f(),this.#y(),this.#v(this.#a.length>0?`已读取 ${this.#a.length} 个本机字体；中文字体已优先排列。`:"浏览器未返回可用本机字体。")}catch{if(t!==this.#m)return;this.#v("未获得本机字体权限；可重试或直接搜索并手动使用字体名称。",!0)}finally{t===this.#m&&(this.#c=!1)}}#v(t,e=!1){if(this.#l.replaceChildren(this.#t.createTextNode(t)),!e)return;let n=c(this.#t,"button","hnr-local-font-retry","重试");n.type="button",n.addEventListener("click",()=>{this.#w()}),this.#l.append(" ",n)}};var Ri=120,xi=5e3,Hi=4e3,Li=1500,Mi=1500,Pi=15,ki=[["refresh","补全评论",["M21 12a9 9 0 0 0-15.219-6.492L3 8","M3 3v5h5","M3 12a9 9 0 0 0 15.219 6.492L21 16","M16 16h5v5"]],["translate","开启或关闭自动翻译",["m5 8 6 6","m4 14 6-6 2-3","M2 5h12","M7 2h1","m22 22-5-10-5 10","M14 18h6"]],["summary","总结讨论",["M15 12H3","M17 18H3","M21 6H3"]],["offline","下载离线 HTML",["M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4","m7 10 5 5 5-5","M12 15V3"]],["history","浏览历史",["M3 12a9 9 0 1 0 3-6.7","M3 3v6h6","M12 7v5l3 2"]],["article","在新标签打开外链",["M15 3h6v6","M10 14 21 3","M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"]],["settings","阅读设置",["M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.38a2 2 0 0 0-.73-2.73l-.15-.09a2 2 0 0 1-1-1.74v-.51a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z","M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"]]],Ni=[["copy-link","复制评论链接",["M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71","M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"]],["translate-comment","翻译此评论",["m5 8 6 6","m4 14 6-6 2-3","M2 5h12","M7 2h1","m22 22-5-10-5 10","M14 18h6"]],["summarize-branch","总结此分支",["M15 12H3","M17 18H3","M21 6H3"]],["reply","在 HN 回复",["m9 17-5-5 5-5","M4 12h12a4 4 0 0 1 4 4v1"]]];function Wr(i,t,e){return i==="translate-comment"&&e?"重新翻译此评论":t}var Oi=["M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"],Fi=["m15 18-6-6 6-6"],Di=1e3,_i=10,$i=9,qi=["m17 11-5-5-5 5","m17 18-5-5-5 5"],zi=["m7 13 5 5 5-5","m7 6 5 5 5-5"];function V(i,t){let e=i.createElementNS("http://www.w3.org/2000/svg","svg");e.setAttribute("viewBox","0 0 24 24"),e.setAttribute("aria-hidden","true"),e.setAttribute("focusable","false");for(let n of t){let r=i.createElementNS("http://www.w3.org/2000/svg","path");r.setAttribute("d",n),e.append(r)}return e}function tt(i,t,e){let n=c(i,"span","hnr-tooltip",t);return n.id=e,n.setAttribute("role","tooltip"),n}var ke=class{constructor(t,e,n,r,o,s,a,l=null){this.document=t;this.actions=o;this.#t=s.child(),this.#A=Br(t),this.#k=e,this.#V=n,this.#e=c(t,"div",""),this.#e.id="hn-reader-root",this.#n=this.#e.attachShadow({mode:"open"});let d=c(t,"style");d.textContent=a,this.#r=c(t,"section","hnr-shell"),this.#r.setAttribute("role","region"),this.#r.setAttribute("aria-labelledby","hnr-reader-title");let h=c(t,"header","hnr-header"),m=c(t,"div","hnr-identity"),p=c(t,"div","hnr-eyebrow","HN READER"),u=c(t,"h1","hnr-title");u.id="hnr-reader-title",this.#o=c(t,"a","hnr-title-jump"),this.#o.href="#hnr-reader-comments",this.#o.dataset.action="scroll-top",this.#o.setAttribute("aria-label","回到评论顶部"),this.#i=tt(t,"回到评论顶部","hnr-tooltip-title-jump"),this.#o.setAttribute("aria-describedby",this.#i.id),this.#s=c(t,"span","hnr-title-original",e.story.title),this.#l=c(t,"span","hnr-title-subtitle"),this.#l.lang="zh-CN",this.#l.hidden=!0,this.#o.append(this.#s,this.#l,this.#i),this.#It(e),u.append(this.#o),m.append(p,u),this.#b=c(t,"span","hnr-coverage");let y=c(t,"div","hnr-header-actions");y.dataset.expanded="false";let f=null,E=c(t,"span","hnr-actions-toggle");E.setAttribute("aria-hidden","true"),E.append(V(t,Fi));let I=c(t,"div","hnr-actions-content"),T=c(t,"nav","hnr-commands");T.setAttribute("aria-label","阅读工具");for(let[j,B,N]of ki){let D=c(t,"button","hnr-command");D.type="button",D.dataset.command=j,D.setAttribute("aria-label",B);let W=tt(t,B,`hnr-tooltip-${j}`);D.setAttribute("aria-describedby",W.id),D.append(V(t,N),W),T.append(D)}let g=c(t,"button","hnr-close");g.type="button",g.dataset.action="close",g.setAttribute("aria-label","退出阅读");let A=tt(t,"退出阅读","hnr-tooltip-close");g.setAttribute("aria-describedby",A.id),g.append(V(t,["M6 6l12 12","M18 6 6 18"]),A);let H=c(t,"a","hnr-original-control");H.href=se(e.story.id),H.target="_blank",H.rel="noopener noreferrer",H.setAttribute("aria-label","回到原帖");let C=tt(t,"回到原帖","hnr-tooltip-original");H.setAttribute("aria-describedby",C.id),H.append(V(t,Oi),C),I.append(T,H,g),y.append(E,I),h.append(m,this.#b,y),this.#a=c(t,"div","hnr-status"),this.#a.setAttribute("role","status"),this.#a.setAttribute("aria-live","polite"),this.#c=c(t,"div","hnr-locator-notice"),this.#c.setAttribute("role","status"),this.#c.setAttribute("aria-live","polite"),this.#c.setAttribute("aria-atomic","true"),this.#c.hidden=!0,this.#u=c(t,"div","hnr-new-comments-notice"),this.#u.setAttribute("role","status"),this.#u.setAttribute("aria-live","polite"),this.#u.setAttribute("aria-atomic","true"),this.#u.hidden=!0;let b=c(t,"button","hnr-new-comments-nav");b.type="button",b.dataset.action="previous-new-comment",b.setAttribute("aria-label","上一条新评论");let v=tt(t,"上一条新评论","hnr-tooltip-previous-new-comment");b.setAttribute("aria-describedby",v.id),b.append(V(t,["m15 18-6-6 6-6"]),v),this.#h=c(t,"button","hnr-new-comments-current"),this.#h.type="button",this.#h.dataset.action="locate-new-comment",this.#m=c(t,"span","hnr-new-comments-label");let R=tt(t,"定位当前新评论","hnr-tooltip-current-new-comment");this.#h.setAttribute("aria-describedby",R.id),this.#h.append(this.#m,R);let L=c(t,"button","hnr-new-comments-nav");L.type="button",L.dataset.action="next-new-comment",L.setAttribute("aria-label","下一条新评论");let z=tt(t,"下一条新评论","hnr-tooltip-next-new-comment");L.setAttribute("aria-describedby",z.id),L.append(V(t,["m9 18 6-6-6-6"]),z);let x=c(t,"button","hnr-new-comments-close");x.type="button",x.dataset.action="dismiss-new-comments",x.setAttribute("aria-label","关闭新评论提示");let O=tt(t,"关闭新评论提示","hnr-tooltip-dismiss-new-comments");x.setAttribute("aria-describedby",O.id),x.append(V(t,["M6 6l12 12","M18 6 6 18"]),O),this.#u.append(b,this.#h,L,x),this.#f=c(t,"div","hnr-comments"),this.#f.id="hnr-reader-comments",this.#f.setAttribute("role","tree"),this.#f.setAttribute("aria-label","Hacker News 评论树"),this.#f.tabIndex=0,this.#r.append(h,this.#a,this.#c,this.#f,this.#u),this.#n.append(d,this.#r),(l??t.body).append(this.#e),this.#p=new Me(this.#f,(j,B)=>this.#Ft(j,B),this.#t,void 0,(j,B,N)=>{this.#Rt(),this.#Mt();let D=this.#f.getBoundingClientRect(),W=D.height>0?[...this.#n.querySelectorAll(".hnr-comment[data-comment-id]")].filter(Q=>{let Pt=Q.getBoundingClientRect();return Pt.bottom>D.top&&Pt.top<D.bottom}).map(Q=>Number.parseInt(Q.dataset.commentId??"",10)).filter(Q=>Number.isSafeInteger(Q)):[],lt=W.length>0?W:B.filter(Q=>Q.kind==="comment").map(Q=>Q.id);this.#st=Object.freeze(lt),this.#ct(lt);let J=lt.join(",");if(J===this.#M){this.#w!==null&&this.document.defaultView?.clearTimeout(this.#w),this.#w=null;return}let K=()=>{this.#t.destroyed||this.#st.join(",")!==J||(this.#M=J,this.actions.onViewportCommentsChanged(Object.freeze(lt)))};this.#w!==null&&this.document.defaultView?.clearTimeout(this.#w),this.#w=null,N?queueMicrotask(K):this.#w=this.document.defaultView?.setTimeout(()=>{this.#w=null,K()},Ri)??null}),this.#t.listen(this.#n,"click",j=>this.#Dt(j)),this.#t.listen(this.#n,"keydown",j=>this.#$t(j));let k=()=>this.#ft();this.#t.listen(this.#f,"wheel",k,{passive:!0}),this.#t.listen(this.#f,"pointerdown",k),this.#t.listen(this.#f,"touchstart",k,{passive:!0});let F=()=>{f!==null&&this.document.defaultView?.clearTimeout(f),f=null};this.#t.listen(y,"pointerenter",()=>{F(),y.dataset.expanded="true",this.#D()}),this.#t.listen(y,"pointerleave",()=>{F(),f=this.document.defaultView?.setTimeout(()=>{y.dataset.expanded="false",f=null,this.#D()},Di)??null}),this.#t.listen(y,"focusin",()=>this.#D()),this.#t.listen(y,"focusout",()=>this.#D());let _=this.document.defaultView;_&&this.#t.listen(_,"resize",()=>this.#D());let Z=_?.ResizeObserver;Z&&this.#t.observe(new Z(()=>this.#D()),this.#o),this.document.fonts&&this.#t.listen(this.document.fonts,"loadingdone",()=>this.#D()),this.#t.listen(this.#n,"focusin",j=>{let B=j.composedPath().find(N=>N instanceof HTMLElement&&N.classList.contains("hnr-comment"));B?.dataset.commentId&&(this.#F=Number.parseInt(B.dataset.commentId,10))}),this.#t.add(()=>this.#e.remove()),this.#t.add(()=>{this.#g!==null&&this.document.defaultView?.clearTimeout(this.#g),this.#g=null,this.#S!==null&&this.document.defaultView?.clearTimeout(this.#S),this.#S=null;for(let j of this.#P.values())this.document.defaultView?.clearTimeout(j);this.#P.clear(),this.#E.clear(),this.#R.length=0,this.#w!==null&&this.document.defaultView?.clearTimeout(this.#w),this.#w=null,this.#v!==null&&this.document.defaultView?.cancelAnimationFrame(this.#v),this.#v=null,this.#d=!1,F(),this.#C+=1,this.#H!==null&&this.document.defaultView?.cancelAnimationFrame(this.#H),this.#H=null,this.#$+=1,this.#T!==null&&this.document.defaultView?.cancelAnimationFrame(this.#T),this.#T=null,this.#ft(),this.#pt(),this.#B(!1)}),this.update(e,n,r),this.#D(),queueMicrotask(()=>this.#o.focus())}#t;#e;#n;#r;#o;#i;#s;#l;#a;#c;#u;#m;#h;#b;#f;#p;#A;#y=new Map;#M="";#w=null;#v=null;#d=!1;#g=null;#S=null;#E=new Set;#P=new Map;#R=[];#x=0;#I=null;#O=null;#H=null;#C=0;#T=null;#$=0;#q=null;#W=0;#N=null;#z=null;#G=null;#K=null;#_=null;#L=null;#nt=null;#X=null;#rt=null;#J=null;#j=null;#U=null;#yt=!1;#Q="original";#F=null;#Y=new Map;#Z=[];#st=Object.freeze([]);#k;#V;get mountedCommentCount(){return this.#p.mountedCount}get surfaceRoot(){return this.#n}captureTopicPosition(){let t=this.#p.capturePosition();return t?{commentId:t.id,offset:t.offset}:null}restoreTopicPosition(t){return this.#p.restorePosition({id:t.commentId,offset:t.offset})}focus(){this.#r.querySelector("button, a, [tabindex]")?.focus()}locateComment(t,e=!0){return this.#it(t,e)}update(t,e,n){this.#k=t,this.#V=e,this.#It(t),this.#s.textContent!==t.story.title&&(this.#s.textContent=t.story.title,this.setTitleTranslation(null),this.#D());let r=t.missingIds().length;this.#b.textContent=`${t.size} 条评论 · ${n&&r===0?"完整":`页面快照${r>0?` · 缺 ${r}`:""}`}`;let o=e.entries();this.#Z=o,this.#p.setEntries(o,this.#Y.size>0?o.map(s=>this.#Y.get(s.id)?.estimatedHeight??112):void 0)}setStatus(t,e="neutral"){this.#g!==null&&this.document.defaultView?.clearTimeout(this.#g),this.#g=null,this.#a.textContent=t,this.#a.dataset.tone=e,this.#a.hidden=t.length===0,e==="success"&&t.length>0&&(this.#g=this.document.defaultView?.setTimeout(()=>{this.#a.textContent===t&&this.#a.dataset.tone==="success"&&(this.#a.hidden=!0),this.#g=null},4e3)??null)}setLocatorNotice(t,e,n=0){this.#S!==null&&this.document.defaultView?.clearTimeout(this.#S),this.#S=null,this.#c.textContent=t,this.#c.dataset.tone=e,this.#c.hidden=t.length===0,!(n<=0||t.length===0)&&(this.#S=this.document.defaultView?.setTimeout(()=>{this.#c.textContent===t&&(this.#c.hidden=!0),this.#S=null},n)??null)}setRealtimeState(t){this.#r.dataset.realtime=t,this.#b.title=t==="connected"?"HN 实时新评论已连接":t==="connecting"?"正在连接 HN 实时新评论":t==="reconnecting"?"HN 实时连接中断，正在重连":"HN 实时新评论当前不可用"}announceNewComments(t){let e=t.filter(r=>this.#k.has(r)&&!this.#R.includes(r));if(e.length===0)return;let n=this.#R.length===0;this.#R.push(...e);for(let r of e)this.#E.add(r);n&&(this.#x=0);for(let r of this.#n.querySelectorAll(".hnr-comment[data-comment-id]")){let o=Number.parseInt(r.dataset.commentId??"",10);this.#E.has(o)&&(r.dataset.newComment="true")}this.#At(),this.#ct(this.#st)}setCommandBusy(t,e){let n=this.#n.querySelector(`[data-command="${t}"]`);n&&(n.disabled=e,n.setAttribute("aria-busy",String(e)))}setTranslation(t,e,n="",r="",o=!0){this.#y.set(t,{text:e,html:n,bilingualHtml:r,complete:o}),this.#at(t)}setTranslations(t){if(t.length!==0)for(let e of t)this.#y.set(e.id,{...e,complete:e.complete??!0}),this.#at(e.id)}setTranslationMode(t){this.#Q=t;for(let e of this.#y.keys())this.#at(e)}setTitleTranslation(t,e="",n=!0){let r=t?.trim()??"";this.#l.dataset.translationComplete=String(n),e?this.#l.innerHTML=e:this.#l.textContent=r,this.#l.hidden=!this.#yt||this.#l.childNodes.length===0,this.#D()}discardIncompleteTranslations(t){let e=t??[...this.#y.keys()];for(let n of e){let r=this.#y.get(n);!r||r.complete||(this.#y.delete(n),this.#at(n))}}setTranslationEnabled(t){this.#yt=t;let e=this.#n.querySelector('[data-command="translate"]');e&&(e.setAttribute("aria-pressed",String(t)),e.dataset.active=String(t)),this.#l.hidden=!t||this.#l.childNodes.length===0,this.#D()}applySettings(t){this.#e.dataset.theme=t.theme;let e=this.document.defaultView?.navigator;Vr(this.#e,t.fontRenderingEnabled,{userAgent:e?.userAgent??"",platform:e?.platform??""}),this.#e.style.setProperty("--hnr-title-font-family",rt(t.titleFontFamily,t.titleCustomFontFamily)),this.#e.style.setProperty("--hnr-content-font-family",rt(t.fontFamily,t.customFontFamily)),this.#e.style.setProperty("--hnr-content-font-weight",String(t.fontWeight)),this.#e.style.setProperty("--hnr-font-scale",String(t.fontScale)),this.#e.style.setProperty("--hnr-line-height",String(t.lineHeight)),this.#e.dataset.translationTheme=t.translationTheme,this.setTranslationEnabled(t.translationEnabled),this.setTranslationMode(t.translationMode),this.#D()}#D(){if(this.#t.destroyed||this.#v!==null||this.#d)return;let t=this.document.defaultView;if(t&&typeof t.requestAnimationFrame=="function"){this.#v=t.requestAnimationFrame(()=>{this.#v=null,this.#gt()});return}this.#d=!0,queueMicrotask(()=>{this.#d=!1,this.#t.destroyed||this.#gt()})}#gt(){let t=this.#o.clientWidth;if(this.#bt(this.#s,t,_i),this.#l.hidden){this.#l.style.removeProperty("font-size");return}this.#bt(this.#l,t,$i)}#bt(t,e,n){if(t.style.removeProperty("font-size"),e<=0||t.scrollWidth<=e)return;let r=this.document.defaultView,o=Number.parseFloat(r?.getComputedStyle(t).fontSize??"");if(!Number.isFinite(o)||o<=0)return;let s=t.scrollWidth,a=Math.max(n,Math.floor(o*e/s*10)/10);t.style.fontSize=`${a}px`;let l=t.scrollWidth;l>e&&a>n&&(a=Math.max(n,Math.floor(a*e/l*10)/10),t.style.fontSize=`${a}px`)}mountedCommentIds(){return Object.freeze([...this.#n.querySelectorAll(".hnr-comment[data-comment-id]")].map(t=>Number.parseInt(t.dataset.commentId??"",10)).filter(t=>Number.isSafeInteger(t)))}viewportCommentIds(){return this.#st}visibleCommentIds(){return Object.freeze(this.#V.entries().filter(t=>t.kind==="comment").map(t=>t.id))}translationRecords(){return new Map([...this.#y].filter(([,t])=>t.complete).map(([t,e])=>[t,{text:e.text,html:e.html,bilingualHtml:e.bilingualHtml}]))}#at(t){let e=this.#n.querySelector(`.hnr-comment[data-comment-id="${t}"]`);if(!e)return;let n=e.querySelector(".hnr-original-text"),r=e.querySelector(".hnr-translated-text"),o=e.querySelector(".hnr-bilingual-text");if(!n||!r||!o)return;let s=this.#y.get(t);s?.html?r.innerHTML=s.html:r.textContent=s?.text??"",s?.bilingualHtml?o.innerHTML=s.bilingualHtml:o.replaceChildren(),r.hidden=!s||this.#Q!=="translated",o.hidden=!s||this.#Q!=="bilingual",n.hidden=!!s&&this.#Q!=="original";let a=e.querySelector('[data-comment-action="translate-comment"]');if(a){let l=Wr("translate-comment","翻译此评论",s?.complete===!0);a.setAttribute("aria-label",l);let d=a.querySelector(".hnr-tooltip");d&&(d.textContent=l)}this.#Ht()}applyPreheat(t){this.#Y=new Map(t);let e=this.#Z;this.#p.seedHeights(e.map(n=>this.#Y.get(n.id)?.estimatedHeight??112)),this.#Ht()}get readerWorkbenchOpen(){return this.#L!==null}openReaderWorkbench(t,e,n,r,o,s,a){this.#dt(),this.#et(),this.#n.querySelector(".hnr-settings-backdrop")?.remove(),this.#B(!1),this.#X=this.#n.activeElement instanceof HTMLElement?this.#n.activeElement:this.#n.querySelector('[data-command="summary"]'),this.#rt=r.onSelectSummary,this.#J=r.onDownload,this.#j=r.onDeleteDownload;let l=c(this.document,"div","hnr-summary-float-layer"),d=c(this.document,"section","hnr-summary-window");d.setAttribute("role","dialog"),d.setAttribute("aria-modal","false"),d.setAttribute("aria-labelledby","hnr-summary-window-title"),d.tabIndex=-1;let h=c(this.document,"header","hnr-summary-window-header"),m=c(this.document,"div","hnr-summary-heading");m.append(c(this.document,"span","hnr-summary-kicker","LOCAL READING WORKBENCH"),c(this.document,"h2","","阅读工作台"),c(this.document,"p","",this.#k.story.title));let p=m.querySelector("h2");p&&(p.id="hnr-summary-window-title");let u=c(this.document,"button","hnr-summary-window-close");u.type="button",u.dataset.action="close-summary",u.setAttribute("aria-label","关闭阅读工作台");let y=tt(this.document,"关闭阅读工作台","hnr-tooltip-workbench-close");u.setAttribute("aria-describedby",y.id),u.append(V(this.document,["M6 6l12 12","M18 6 6 18"]),y),h.append(m,u);let f=c(this.document,"div","hnr-summary-tabs");f.setAttribute("role","tablist"),f.setAttribute("aria-label","阅读工作台");let E=c(this.document,"button","hnr-summary-tab","讨论总结"),I=c(this.document,"button","hnr-summary-tab",`总结历史 ${t.length}`),T=c(this.document,"button","hnr-summary-tab",`下载历史 ${e.length}`),g=c(this.document,"button","hnr-summary-tab",`浏览历史 ${n.length}`);for(let[N,D]of[[E,"insight"],[I,"summary-history"],[T,"downloads"],[g,"browsing-history"]])N.type="button",N.dataset.summaryTab=D,N.setAttribute("role","tab"),N.setAttribute("aria-selected",String(D===o)),f.append(N);let A=c(this.document,"div","hnr-summary-window-body"),H=c(this.document,"section","hnr-summary-pane hnr-summary-insight-pane");H.dataset.summaryPane="insight",H.setAttribute("role","tabpanel");let C=c(this.document,"form","hnr-summary-controls"),b=c(this.document,"div","hnr-summary-controls-heading");b.append(c(this.document,"strong","","生成新总结"),c(this.document,"span","","确认后才会调用已配置的 AI"));let v=(N,D)=>{let W=c(this.document,"label","hnr-summary-field");return W.append(c(this.document,"span","",N),D),W},R=c(this.document,"select");R.name="scope";for(let[N,D,W]of[["all","全部已加载评论",!1],["branch",this.#F?`当前分支 #${this.#F}`:"当前分支（先聚焦一条评论）",!this.#F]]){let lt=c(this.document,"option","",D);lt.value=N,lt.disabled=W,R.append(lt)}let L=c(this.document,"select");L.name="length";for(let[N,D]of[["short","精简"],["standard","标准"],["detailed","详细"]]){let W=c(this.document,"option","",D);W.value=N,W.selected=N==="standard",L.append(W)}let z=c(this.document,"button","hnr-summary-run","开始总结");z.type="submit";let x=c(this.document,"span","hnr-summary-progress-label","");x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),C.append(b,v("范围",R),v("长度",L),z,x);let O=c(this.document,"div","hnr-summary-result");H.append(C,O);let k=c(this.document,"section","hnr-summary-pane hnr-summary-history-pane");k.dataset.summaryPane="summary-history",k.setAttribute("role","tabpanel"),k.hidden=!0;let F=c(this.document,"section","hnr-summary-pane hnr-download-pane");F.dataset.summaryPane="downloads",F.setAttribute("role","tabpanel"),F.hidden=!0;let _=c(this.document,"section","hnr-summary-pane hnr-browsing-history-pane");_.dataset.summaryPane="browsing-history",_.setAttribute("role","tabpanel"),_.hidden=!0,A.append(H,k,F,_),d.append(h,f,A),l.append(d),this.#r.append(l),this.#L=l;let Z=N=>{let D=N;D.key!=="Escape"||!l.isConnected||(D.preventDefault(),D.stopPropagation(),this.#B())};this.#nt=Z,this.document.addEventListener("keydown",Z,!0),l.addEventListener("click",N=>{N.target===l&&this.#B()});let j=N=>{for(let D of f.querySelectorAll(".hnr-summary-tab"))D.setAttribute("aria-selected",String(D.dataset.summaryTab===N));H.hidden=N!=="insight",k.hidden=N!=="summary-history",F.hidden=N!=="downloads",_.hidden=N!=="browsing-history"};E.addEventListener("click",()=>j("insight")),I.addEventListener("click",()=>j("summary-history")),T.addEventListener("click",()=>j("downloads")),g.addEventListener("click",()=>j("browsing-history")),C.addEventListener("submit",N=>{N.preventDefault();let D=R.value==="branch"&&this.#F?{kind:"branch",rootId:this.#F}:{kind:"all"};r.onRunSummary(D,L.value)});let B=s;B?(this.#U=B.id,this.#ht(O,B),r.onSelectSummary(B)):(this.#U=null,this.#Pt(O)),this.#vt(k,t,j),this.#Tt(F,e,a??null),this.#kt(_,n,r.onOpenTopic),j(o),queueMicrotask(()=>d.focus())}showSummary(t,e){let n=this.#L;if(!n)return;let r=n.querySelector(".hnr-summary-result"),o=n.querySelector(".hnr-summary-history-pane");if(!r||!o)return;this.#U=t.id,this.#ht(r,t),this.#vt(o,e,a=>{for(let l of n.querySelectorAll(".hnr-summary-tab"))l.setAttribute("aria-selected",String(l.dataset.summaryTab===a));for(let l of n.querySelectorAll(".hnr-summary-pane"))l.hidden=l.dataset.summaryPane!==a});let s=n.querySelector('[data-summary-tab="summary-history"]');s&&(s.textContent=`总结历史 ${e.length}`),this.setSummaryBusy(!1)}updateOfflineDownloads(t,e){let n=this.#L;if(!n)return;let r=n.querySelector(".hnr-download-pane");if(!r)return;this.#Tt(r,e,t);let o=n.querySelector('[data-summary-tab="downloads"]');o&&(o.textContent=`下载历史 ${e.length}`)}setSummaryBusy(t){let e=this.#L;if(!e)return;e.querySelector(".hnr-summary-window")?.setAttribute("aria-busy",String(t));for(let r of e.querySelectorAll(".hnr-summary-controls button, .hnr-summary-controls select"))r.disabled=t;let n=e.querySelector(".hnr-summary-progress-label");n&&t?(n.dataset.tone="busy",n.textContent="正在理解评论树…"):n?.dataset.tone==="busy"&&(n.dataset.tone="neutral",n.textContent="")}setSummaryNotice(t,e="neutral"){let n=this.#L?.querySelector(".hnr-summary-progress-label");n&&(n.dataset.tone=e,n.textContent=t)}#Pt(t){t.replaceChildren();let e=c(this.document,"div","hnr-summary-empty");e.append(c(this.document,"span","hnr-summary-empty-mark","01"),c(this.document,"h3","","还没有讨论摘要"),c(this.document,"p","","选择总结范围与长度，开始后会在这里生成覆盖率、共识、分歧和关键分支。")),t.append(e)}#ht(t,e){t.replaceChildren();let n=this.#L?.querySelector(".hnr-summary-heading p");n&&(n.textContent=e.storyTitle);let{summary:r}=e,o=Math.max(0,r.availableComments),s=Math.max(0,Math.min(r.includedComments,o)),a=o>0?Math.round(s/o*100):0,l=c(this.document,"div","hnr-summary-dashboard"),d=c(this.document,"section","hnr-summary-coverage-card"),h=c(this.document,"div","hnr-summary-coverage-ring");h.style.setProperty("--hnr-summary-coverage",`${a}%`),h.setAttribute("role","img"),h.setAttribute("aria-label",`总结覆盖 ${s}/${o} 条评论，${a}%`),h.append(c(this.document,"strong","",`${a}%`),c(this.document,"span","","覆盖率"));let m=c(this.document,"div","hnr-summary-coverage-copy");m.append(c(this.document,"span","hnr-summary-overline","COMMENT COVERAGE"),c(this.document,"strong","",`${s} / ${o} 条评论`),c(this.document,"p","",r.coverageNote)),d.append(h,m);let p=c(this.document,"div","hnr-summary-metrics");for(let[E,I,T]of[[r.consensus.length,"共识","consensus"],[r.disputes.length,"分歧","disputes"],[r.branches.length,"关键分支","branches"]]){let g=c(this.document,"div","hnr-summary-metric");g.dataset.tone=T,g.append(c(this.document,"strong","",String(E)),c(this.document,"span","",I)),p.append(g)}l.append(d,p);let u=c(this.document,"section","hnr-summary-overview");u.append(c(this.document,"span","hnr-summary-section-index","01 / OVERVIEW"),c(this.document,"h3","","讨论全景"),c(this.document,"p","",r.overview));let y=c(this.document,"div","hnr-summary-signal-grid");if(y.append(this.#wt("共识","反复出现的一致观点",r.consensus,"consensus"),this.#wt("分歧","仍在交锋的关键判断",r.disputes,"disputes")),t.append(l,u,y),r.branches.length>0){let E=c(this.document,"section","hnr-summary-branches"),I=c(this.document,"div","hnr-summary-section-heading");I.append(c(this.document,"span","hnr-summary-section-index","04 / BRANCHES"),c(this.document,"h3","","关键分支")),E.append(I);for(let T of r.branches){let g=e.storyId===this.#k.story.id?c(this.document,"button","hnr-summary-branch"):c(this.document,"a","hnr-summary-branch");g instanceof HTMLButtonElement?(g.type="button",g.dataset.action="locate-summary-comment",g.dataset.commentId=String(T.commentId)):(g.href=`https://news.ycombinator.com/item?id=${T.commentId}`,g.target="_blank",g.rel="noopener noreferrer",g.setAttribute("aria-label",`在新标签打开历史总结中的评论 #${T.commentId}`)),g.append(c(this.document,"span","hnr-summary-branch-id",`#${T.commentId}`),c(this.document,"span","",T.summary),V(this.document,["M5 12h14","m13 6 6 6-6 6"])),E.append(g)}t.append(E)}let f=c(this.document,"footer","hnr-summary-meta");f.append(c(this.document,"span","",this.#St(e.scope)),c(this.document,"span","",this.#Et(e.length)),c(this.document,"span","",e.model||"未标记模型"),c(this.document,"time","",this.#lt(e.savedAt))),t.append(f)}#wt(t,e,n,r){let o=c(this.document,"section","hnr-summary-signal");if(o.dataset.tone=r,o.append(c(this.document,"span","hnr-summary-section-index",r==="consensus"?"02 / SIGNAL":"03 / TENSION"),c(this.document,"h3","",t),c(this.document,"p","hnr-summary-signal-description",e)),n.length===0)return o.append(c(this.document,"p","hnr-summary-none","本次总结未提取到此类信号。")),o;let s=c(this.document,"ol");for(let a of n)s.append(c(this.document,"li","",a));return o.append(s),o}#vt(t,e,n){t.replaceChildren();let r=c(this.document,"div","hnr-summary-history-heading");if(r.append(c(this.document,"span","hnr-summary-section-index","ARCHIVE / 30 DAYS"),c(this.document,"h3","","全部讨论总结历史"),c(this.document,"p","","汇总所有帖子近 30 天的讨论总结，按最近使用排列并自动去重。")),t.append(r),e.length===0){t.append(c(this.document,"p","hnr-summary-history-empty","生成第一份总结后，记录会出现在这里。"));return}let o=c(this.document,"div","hnr-summary-history-list");for(let[s,a]of e.entries()){let l=c(this.document,"button","hnr-summary-history-entry");l.type="button",l.dataset.active=String(a.id===this.#U),l.append(c(this.document,"span","hnr-summary-history-number",String(s+1).padStart(2,"0")),c(this.document,"strong","hnr-summary-history-story",a.storyTitle),c(this.document,"span","hnr-summary-history-overview",a.summary.overview));let d=c(this.document,"span","hnr-summary-history-meta");d.append(c(this.document,"span","",this.#St(a.scope)),c(this.document,"span","",this.#Et(a.length)),c(this.document,"span","",`${a.summary.includedComments}/${a.summary.availableComments} 条`),c(this.document,"time","",this.#lt(a.savedAt))),l.append(d),l.addEventListener("click",()=>{let h=this.#L?.querySelector(".hnr-summary-result");if(h){this.#U=a.id,this.#ht(h,a),this.#rt?.(a),n("insight");for(let m of o.querySelectorAll(".hnr-summary-history-entry"))m.dataset.active=String(m===l)}}),o.append(l)}t.append(o)}#kt(t,e,n){t.replaceChildren();let r=c(this.document,"div","hnr-summary-history-heading");if(r.append(c(this.document,"span","hnr-summary-section-index","LOCAL / READING TRAIL"),c(this.document,"h3","","浏览历史"),c(this.document,"p","","按最近浏览排列；返回 Topic 时同步恢复回复树收纳状态和离开位置。")),t.append(r),e.length===0){t.append(c(this.document,"p","hnr-summary-history-empty","打开第一篇 Topic 后，浏览记录会出现在这里。"));return}let o=c(this.document,"input","hnr-browsing-history-search");o.type="search",o.placeholder="搜索标题、Topic ID 或评论 ID",o.setAttribute("aria-label","搜索浏览历史");let s=c(this.document,"div","hnr-browsing-history-list"),a=[];for(let[d,h]of e.entries()){let m=c(this.document,"button","hnr-browsing-history-entry");m.type="button",m.dataset.storyId=String(h.storyId),m.dataset.active=String(h.storyId===this.#k.story.id),m.append(c(this.document,"span","hnr-summary-history-number",String(d+1).padStart(2,"0")),c(this.document,"strong","hnr-browsing-history-story",h.storyTitle));let p=c(this.document,"span","hnr-browsing-history-meta");p.append(c(this.document,"span","",`Topic #${h.storyId}`),c(this.document,"time","",h.visitedAt>0?this.#lt(h.visitedAt):"较早记录")),h.position&&p.append(c(this.document,"span","",`评论 #${h.position.commentId}`)),h.collapsedCommentCount>0&&p.append(c(this.document,"span","",`收纳 ${h.collapsedCommentCount} 个分支`)),m.append(p),m.addEventListener("click",()=>n(h.storyId)),a.push({button:m,identity:`${h.storyTitle} ${h.storyId} ${h.position?.commentId??""}`.toLocaleLowerCase()}),s.append(m)}let l=()=>{let d=o.value.trim().toLocaleLowerCase();for(let h of a)h.button.hidden=d.length>0&&!h.identity.includes(d)};o.addEventListener("input",l),t.append(o,s)}#Tt(t,e,n){t.replaceChildren();let r=c(this.document,"div","hnr-summary-history-heading");if(r.append(c(this.document,"span","hnr-summary-section-index","OFFLINE / LOCAL ONLY"),c(this.document,"h3","","离线 HTML"),c(this.document,"p","","可视化准备译文、生成 HTML 和本地保存；完成记录保留 30 天。")),t.append(r),n){let a=c(this.document,"section","hnr-download-progress");a.dataset.status=n.status;let l=c(this.document,"div","hnr-download-progress-copy");l.append(c(this.document,"span","hnr-summary-section-index",n.status==="ready"?"READY":n.status==="error"?"INTERRUPTED":"IN PROGRESS"),c(this.document,"strong","",n.storyTitle),c(this.document,"p","",n.message));let d=["translating","generating","saving"].indexOf(n.stage),h=c(this.document,"ol","hnr-download-stages");for(let[m,[p,u,y]]of[["translating","准备译文",n.total>0?`${n.complete}/${n.total}`:"检查本地译文"],["generating","生成 HTML","组装安全的离线文档"],["saving","保存记录","写入本地历史并下载"]].entries()){let f=c(this.document,"li","hnr-download-stage"),E=n.status==="ready"||m<d?"done":m===d?n.status==="error"?"error":"active":"pending";if(f.dataset.state=E,f.dataset.stage=p,f.append(c(this.document,"span","hnr-download-stage-mark",E==="done"?"✓":String(m+1).padStart(2,"0")),c(this.document,"strong","",u),c(this.document,"span","",y)),p==="translating"&&n.total>0){let I=Math.max(0,Math.min(1,n.complete/n.total)),T=c(this.document,"span","hnr-download-meter");T.setAttribute("role","progressbar"),T.setAttribute("aria-label","离线译文准备进度"),T.setAttribute("aria-valuemin","0"),T.setAttribute("aria-valuemax",String(n.total)),T.setAttribute("aria-valuenow",String(n.complete)),T.style.setProperty("--hnr-download-progress",`${Math.round(I*100)}%`),f.append(T)}h.append(f)}a.append(l,h),t.append(a)}else{let a=c(this.document,"div","hnr-download-idle");a.append(c(this.document,"strong","","当前没有下载任务"),c(this.document,"span","","点击阅读器顶部的下载按钮，会在这里显示翻译与 HTML 生成进度。")),t.append(a)}let o=c(this.document,"div","hnr-download-history-heading");if(o.append(c(this.document,"strong","",`全部下载历史 · ${e.length}`),c(this.document,"span","","跨帖子保存已完成的 HTML，可随时再次下载。")),t.append(o),e.length===0){t.append(c(this.document,"p","hnr-summary-history-empty","首个离线 HTML 完成后，文件会保存在这里。"));return}let s=c(this.document,"div","hnr-download-history-list");for(let[a,l]of e.entries()){let d=c(this.document,"article","hnr-download-history-entry"),h=c(this.document,"div","hnr-download-history-detail");h.append(c(this.document,"strong","",l.storyTitle),c(this.document,"span","",l.filename));let m=c(this.document,"div","hnr-download-history-meta");m.append(c(this.document,"span","",`${l.commentCount} 条评论`),c(this.document,"span","",`${l.translatedCount} 条译文`),c(this.document,"span","",this.#Nt(l.bytes)),c(this.document,"time","",this.#lt(l.savedAt)));let p=c(this.document,"div","hnr-download-history-actions"),u=c(this.document,"button","hnr-download-history-action");u.type="button",u.setAttribute("aria-label",`再次下载 ${l.storyTitle} 的离线 HTML`);let y=tt(this.document,`再次下载 ${l.filename}`,`hnr-tooltip-download-${a}`);u.setAttribute("aria-describedby",y.id),u.append(V(this.document,["M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4","m7 10 5 5 5-5","M12 15V3"]),c(this.document,"span","","下载"),y),u.addEventListener("click",()=>this.#J?.(l));let f=c(this.document,"button","hnr-download-history-action hnr-download-history-delete");f.type="button",f.setAttribute("aria-label",`删除 ${l.storyTitle} 的下载历史`);let E=c(this.document,"span","","删除"),I=tt(this.document,`删除 ${l.filename}`,`hnr-tooltip-delete-download-${a}`);f.setAttribute("aria-describedby",I.id),f.append(V(this.document,["M3 6h18","M8 6V4h8v2","m19 6-1 14H6L5 6","M10 11v5","M14 11v5"]),E,I);let T=!1,g=()=>{T=!1,f.dataset.confirm="false",E.textContent="删除",f.setAttribute("aria-label",`删除 ${l.storyTitle} 的下载历史`)};f.addEventListener("click",()=>{if(T){this.#j?.(l);return}T=!0,f.dataset.confirm="true",E.textContent="确认删除",f.setAttribute("aria-label",`确认删除 ${l.storyTitle} 的下载历史`)}),f.addEventListener("blur",g),p.append(u,f),d.append(h,m,p),s.append(d)}t.append(s)}#Nt(t){return t<1024?`${t} B`:t<1024*1024?`${(t/1024).toFixed(1)} KB`:`${(t/(1024*1024)).toFixed(1)} MB`}#St(t){return t.kind==="all"?"全帖":`分支 #${t.rootId}`}#Et(t){return t==="short"?"精简":t==="detailed"?"详细":"标准"}#lt(t){return new Intl.DateTimeFormat("zh-CN",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:!1}).format(t)}#B(t=!0){this.#Ot(),this.#L?.remove(),this.#L=null,this.#rt=null,this.#J=null,this.#j=null,this.#U=null;let e=this.#X;this.#X=null,t&&e?.isConnected&&e.focus({preventScroll:!0})}#Ot(){let t=this.#nt;t&&(this.#nt=null,this.document.removeEventListener("keydown",t,!0))}openSettings(t,e){this.#tt(),this.#dt(),this.#et(),this.#B(!1),this.#n.querySelector(".hnr-settings-backdrop")?.remove();let n=c(this.document,"div","hnr-settings-backdrop"),r=c(this.document,"form","hnr-settings hnr-settings-popover");r.setAttribute("role","dialog"),r.setAttribute("aria-modal","true");let o=[{id:"reading",group:"阅读",title:"阅读与翻译",description:"设置自动翻译、翻译服务和正文显示方式。",icon:["M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z","M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"]},{id:"font",group:"阅读",title:"字体与排版",description:"分别调整 Reader 标题与评论正文的字体，并设置正文字重、字号和行高。",icon:["M4 7V4h16v3","M9 20h6","M12 4v16"]},{id:"ai",group:"服务",title:"AI 服务",description:"配置 OpenAI-compatible 模型，仅在明确选择或触发时请求。",icon:["m12 3-1.4 3.6L7 8l3.6 1.4L12 13l1.4-3.6L17 8l-3.6-1.4z","m19 14-.8 2.2L16 17l2.2.8L19 20l.8-2.2L22 17l-2.2-.8z"]},{id:"storage",group:"数据",title:"本地数据",description:"清理缓存与本地历史，或把所有阅读设置恢复为默认值。",icon:["M4 6c0-1.7 3.6-3 8-3s8 1.3 8 3-3.6 3-8 3-8-1.3-8-3z","M4 6v6c0 1.7 3.6 3 8 3s8-1.3 8-3V6","M4 12v6c0 1.7 3.6 3 8 3s8-1.3 8-3v-6"]}],s=(w,S)=>{let M=c(this.document,"label","hnr-field");return M.append(c(this.document,"span","",w),S),M},a=(w,S,M)=>{let P=c(this.document,"select");P.name=w;for(let[$,Y]of M){let St=c(this.document,"option","",Y);St.value=$,St.selected=$===S,P.append(St)}return P},l=(w,S,M="text")=>{let P=c(this.document,"input");return P.name=w,P.type=M,P.value=S,P.autocomplete=M==="password"?"new-password":"off",P},d=(w,S,M,P,$)=>{let Y=l(w,String(S));return Y.type="number",Y.min=String(M),Y.max=String(P),Y.step=String($),Y},h=(w,S,M,P,$)=>{let Y=d(w,S,M,P,$);return Y.type="range",Y},m=new Map,p=new Map,u=new Map,y=new Map,f="reading",E=c(this.document,"aside","hnr-settings-tabs"),I=c(this.document,"div","hnr-settings-brand"),T=c(this.document,"span","hnr-settings-brand-mark","Y");T.setAttribute("aria-hidden","true");let g=c(this.document,"span","hnr-settings-brand-name");for(let w of["HACKER","NEWS","READER"])g.append(c(this.document,"span","",w));I.append(T,g);let A=c(this.document,"div","hnr-settings-search-shell"),H=c(this.document,"label","hnr-settings-search");H.append(V(this.document,["M21 21l-4.35-4.35","M19 11a8 8 0 1 1-16 0 8 8 0 0 1 16 0z"]));let C=l("","");C.removeAttribute("name"),C.type="search",C.placeholder="搜索设置…",C.setAttribute("aria-label","搜索设置");let b=c(this.document,"button","hnr-settings-search-clear");b.type="button",b.hidden=!0,b.setAttribute("aria-label","清空设置搜索"),b.append(V(this.document,["M6 6l12 12","M18 6 6 18"])),H.append(C,b);let v=c(this.document,"span","hnr-settings-search-status","输入名称或功能即可筛选");v.setAttribute("role","status"),A.append(H,v),I.append(A);let R=c(this.document,"div","hnr-settings-nav-shell"),L=c(this.document,"nav","hnr-settings-nav");L.setAttribute("role","tablist"),L.setAttribute("aria-label","设置分类");for(let w of["阅读","服务","数据"]){let S=c(this.document,"div","hnr-settings-nav-group");S.dataset.settingsGroup=w,S.append(c(this.document,"span","hnr-settings-nav-group-label",w));for(let M of o.filter(P=>P.group===w)){let P=c(this.document,"button","hnr-settings-tab");P.type="button",P.id=`hnr-settings-tab-${M.id}`,P.dataset.settingsPanel=M.id,P.setAttribute("role","tab"),P.setAttribute("aria-controls",`hnr-settings-panel-${M.id}`),P.append(V(this.document,M.icon),c(this.document,"span","",M.title)),m.set(M.id,P),S.append(P)}y.set(w,S),L.append(S)}R.append(L);let z=c(this.document,"div","hnr-settings-sidebar-footer"),x=a("theme",t.theme,[["auto","跟随系统"],["light","浅色"],["dark","深色"]]);x.setAttribute("aria-label","阅读器主题"),z.append(c(this.document,"span","","主题"),x),E.append(I,R,z);let O=c(this.document,"div","hnr-settings-panel"),k=c(this.document,"div","hnr-settings-pages"),F=c(this.document,"div","hnr-settings-search-empty");F.hidden=!0,F.append(V(this.document,["M21 21l-4.35-4.35","M19 11a8 8 0 1 1-16 0 8 8 0 0 1 16 0z"]),c(this.document,"strong","","没有找到匹配的设置"),c(this.document,"span","","试试“字体”“翻译”“AI”或“缓存”。")),k.append(F);for(let w of o){let S=c(this.document,"section","hnr-settings-section");S.id=`hnr-settings-panel-${w.id}`,S.dataset.settingsPanel=w.id,S.setAttribute("role","tabpanel"),S.setAttribute("aria-labelledby",`hnr-settings-tab-${w.id}`);let M=c(this.document,"div","hnr-settings-intro"),P=c(this.document,"h2","hnr-settings-title",w.title);P.id=`hnr-settings-title-${w.id}`,M.append(P,c(this.document,"p","hnr-settings-description",w.description));let $=c(this.document,"div","hnr-settings-content");$.dataset.settingsContent=w.id,S.append(M,$),p.set(w.id,S),u.set(w.id,$),k.append(S)}let _=Object.freeze({quote:"淡灰引用",plain:"自然正文",weakening:"弱化译文","dividing-line":"分隔线",underline:"下划线",highlight:"柔和高亮",paper:"纸张卡片"}),Z=a("translationTheme",t.translationTheme,nn.map(w=>[w,_[w]]));Z.setAttribute("aria-label","译文呈现样式");let j=c(this.document,"span","hnr-translation-theme-preview hnr-bilingual-text");j.setAttribute("aria-label","译文样式效果预览"),j.append(c(this.document,"span","hnr-bilingual-original-section","Knowledge grows when ideas are shared."),c(this.document,"span","hnr-bilingual-translation-section","知识会在分享中不断生长。"));let B=c(this.document,"span","hnr-translation-theme-control");B.append(Z);let N=s("译文样式",B);N.classList.add("hnr-translation-theme-field"),N.append(j);let D=c(this.document,"fieldset","hnr-settings-card hnr-reading-settings");D.append(c(this.document,"legend","","翻译阅读"),s("翻译服务",a("provider",t.translationProvider,[["auto","公共自动"],["google","Google"],["microsoft","Microsoft"],["ai","自定义 AI"]])),s("显示方式",a("mode",t.translationMode,[["original","仅原文"],["bilingual","双语"],["translated","仅译文"]])),s("滚动预翻译",a("translationEnabled",t.translationEnabled?"on":"off",[["on","开启（持久化）"],["off","关闭"]])),N),u.get("reading")?.append(D);let W=l("aiBaseUrl",t.ai.baseUrl),lt=l("aiApiKey",t.ai.apiKey,"password"),J=l("aiModel",t.ai.model),K=c(this.document,"select","hnr-model-select");K.disabled=!0,K.setAttribute("aria-label","选择已获取模型");let Q=c(this.document,"button","hnr-load-models","获取模型");Q.type="button";let Pt=c(this.document,"div","hnr-model-picker");Pt.append(J,K,Q);let Kt=c(this.document,"textarea");Kt.name="aiPrompt",Kt.rows=3,Kt.value=t.ai.prompt;let $n=c(this.document,"fieldset","hnr-settings-card hnr-ai-settings");$n.append(c(this.document,"legend","","自定义 AI（OpenAI-compatible）"),s("Base URL",W),s("API Key",lt),s("模型",Pt),s("附加提示词",Kt),s("RPM（0 为不限）",d("aiRpm",t.ai.requestsPerMinute,0,1e4,1)),s("TPM（0 为不限）",d("aiTpm",t.ai.tokensPerMinute,0,1e7,100))),u.get("ai")?.append($n);let qn=e.queryLocalFonts??this.#A,We=null,Ge=null,zn=qn?async()=>{if(We)return We;Ge??=qn().then(w=>(We=w,w));try{return await Ge}finally{Ge=null}}:void 0,dt=new Gt({document:this.document,fontFamily:t.titleFontFamily,customFontFamily:t.titleCustomFontFamily,fontFamilyName:"titleFontFamily",customFontFamilyName:"titleCustomFontFamily",queryLocalFonts:zn}),ct=new Gt({document:this.document,fontFamily:t.fontFamily,customFontFamily:t.customFontFamily,queryLocalFonts:zn}),Jt=a("fontRenderingEnabled",t.fontRenderingEnabled?"on":"off",[["on","开启"],["off","关闭"]]),Qt=a("fontWeight",String(t.fontWeight),ln.map(w=>[String(w),`${w===300?"细":w===400?"常规":w===500?"中等":"半粗"} ${w}`])),Yt=h("fontScale",t.fontScale,.85,1.35,.01),Xt=h("lineHeight",t.lineHeight,1.35,2,.01);Jt.setAttribute("aria-label","字体显示优化"),Qt.setAttribute("aria-label","评论正文字重"),Yt.setAttribute("aria-label","评论正文字号"),Xt.setAttribute("aria-label","评论正文行高");let jn=c(this.document,"output","hnr-font-range-value"),Un=c(this.document,"output","hnr-font-range-value"),vt=(w,S,M)=>{let P=c(this.document,"div","hnr-font-setting-row"),$=c(this.document,"span","hnr-font-setting-copy");return $.append(c(this.document,"strong","",w),c(this.document,"small","",S)),P.append($,M),P},Vn=c(this.document,"span","hnr-font-option-control");Vn.append(Qt);let Bn=c(this.document,"span","hnr-font-option-control");Bn.append(Jt);let Wn=c(this.document,"span","hnr-font-range-control");Wn.append(Yt,jn);let Gn=c(this.document,"span","hnr-font-range-control");Gn.append(Xt,Un);let Kn=c(this.document,"div","hnr-font-preview");Kn.append(c(this.document,"strong","hnr-font-title-preview","Hacker News 标题字体预览"),c(this.document,"span","hnr-font-body-preview","评论正文预览：春江潮水连海平。The quick brown fox jumps over 0123456789."));let Jn=c(this.document,"fieldset","hnr-settings-card hnr-font-settings");Jn.append(c(this.document,"legend","","阅读排版"),vt("字体显示优化","在 Reader 中启用内置字体平滑、描边和阴影优化。",Bn),vt("标题字体","用于 Reader 顶部故事标题与中文译题。",dt.element),vt("正文字体","自动读取浏览器可用字体；支持搜索、选择和即时预览。",ct.element),vt("字重","仅影响评论正文。",Vn),vt("字号","85% – 135%",Wn),vt("行高","1.35 – 2.00",Gn),Kn),u.get("font")?.append(Jn);let Qn=c(this.document,"section","hnr-settings-card hnr-storage-settings"),Yn=c(this.document,"div","hnr-storage-copy");Yn.append(c(this.document,"strong","","缓存与设置"),c(this.document,"span","","清理会删除总结与下载历史，但不会删除 API Key；恢复默认会重置完整设置。"));let Xn=c(this.document,"div","hnr-storage-actions"),kt=c(this.document,"button","","清理缓存与历史");kt.type="button";let Ke=c(this.document,"button","hnr-danger","恢复默认");Ke.type="button",Xn.append(kt,Ke),Qn.append(Yn,Xn),u.get("storage")?.append(Qn);let Tt=c(this.document,"div","hnr-settings-status","更改将在保存后生效；字体排版可实时预览。");Tt.setAttribute("role","status"),Tt.setAttribute("aria-live","polite");let Zn=c(this.document,"div","hnr-settings-draft-bar"),tr=c(this.document,"div","hnr-settings-actions"),Je=c(this.document,"button","hnr-settings-cancel","取消");Je.type="button";let er=c(this.document,"button","hnr-primary","保存全部更改");er.type="submit",tr.append(Je,er),Zn.append(Tt,tr),O.append(k,Zn);let Nt=c(this.document,"button","hnr-settings-close");Nt.type="button",Nt.setAttribute("aria-label","关闭设置"),Nt.append(V(this.document,["M6 6l12 12","M18 6 6 18"])),r.append(E,O,Nt),n.append(r),this.#r.append(n),this.#K=()=>{dt.destroy(),ct.destroy(),n.remove()};let gt=(w,S="neutral")=>{Tt.textContent=w,Tt.dataset.tone=S,Tt.setAttribute("role",S==="error"?"alert":"status")},Ot=w=>{f=w,w==="font"&&(dt.activate(),ct.activate());for(let S of o){let M=S.id===w,P=m.get(S.id),$=p.get(S.id);P&&(P.classList.toggle("active",M),P.setAttribute("aria-selected",String(M)),P.tabIndex=M?0:-1),$&&($.hidden=!M)}r.setAttribute("aria-labelledby",`hnr-settings-title-${w}`),k.scrollTop=0};for(let[w,S]of m)S.addEventListener("click",()=>Ot(w));L.addEventListener("keydown",w=>{if(!(w instanceof KeyboardEvent)||w.key!=="ArrowDown"&&w.key!=="ArrowUp")return;let S=[...m.values()].filter(Y=>!Y.hidden),M=S.indexOf(this.#n.activeElement),P=w.key==="ArrowDown"?1:-1,$=S[(M+P+S.length)%S.length];$?.focus(),$?.dataset.settingsPanel&&Ot($.dataset.settingsPanel),w.preventDefault()});let nr=()=>{let w=C.value.trim().toLocaleLowerCase(),S=0;for(let M of o){let P=p.get(M.id),$=`${M.title} ${M.description} ${P?.textContent??""}`.toLocaleLowerCase(),Y=!w||$.includes(w),St=m.get(M.id);St&&(St.hidden=!Y),Y&&(S+=1)}for(let[M,P]of y)P.hidden=!o.some($=>$.group===M&&!m.get($.id)?.hidden);if(b.hidden=w.length===0,F.hidden=S>0,v.textContent=w?S>0?`找到 ${S} 个设置分区`:"没有匹配结果":"输入名称或功能即可筛选",m.get(f)?.hidden){let M=o.find(P=>!m.get(P.id)?.hidden);M&&Ot(M.id)}};C.addEventListener("input",nr),b.addEventListener("click",()=>{C.value="",nr(),C.focus()}),Ot(f),Q.addEventListener("click",()=>{Q.disabled=!0,gt("正在读取 /models…");let w={...t.ai,baseUrl:W.value,apiKey:lt.value,model:J.value};e.onLoadModels(w).then(S=>{K.replaceChildren(...S.map(M=>{let P=c(this.document,"option");return P.value=M,P.textContent=M,P.selected=M===J.value,P})),!J.value.trim()&&S[0]&&(J.value=S[0]),K.disabled=S.length===0,S.includes(J.value)?K.value=J.value:K.selectedIndex=-1,gt(`已获取 ${S.length} 个模型。`,"success")}).catch(S=>{gt(S instanceof Error?S.message:"模型列表获取失败","error")}).finally(()=>{Q.disabled=!1})}),K.addEventListener("change",()=>{K.value&&(J.value=K.value)}),J.addEventListener("input",()=>{[...K.options].some(w=>w.value===J.value)?K.value=J.value:K.selectedIndex=-1});let Ft=()=>{let w=$t({...t,theme:x.value,titleFontFamily:dt.fontFamilyInput.value,titleCustomFontFamily:dt.customFontFamilyInput.value,fontFamily:ct.fontFamilyInput.value,customFontFamily:ct.customFontFamilyInput.value,fontRenderingEnabled:Jt.value==="on",fontWeight:Qt.value,fontScale:Yt.value,lineHeight:Xt.value,translationTheme:Z.value});jn.value=`${Math.round(w.fontScale*100)}%`,Un.value=w.lineHeight.toFixed(2),this.applySettings(w),e.onThemePreview?.(w.theme),e.onSettingsPreview?.(w)};this.#G=()=>{this.applySettings(t),e.onThemePreview?.(t.theme),e.onSettingsPreview?.(t)};for(let w of[Jt,Qt,Yt,Xt,Z])w.addEventListener(w.tagName==="SELECT"?"change":"input",Ft);dt.fontFamilyInput.addEventListener("change",Ft),ct.fontFamilyInput.addEventListener("change",Ft),x.addEventListener("change",Ft),Ft();let Zt=()=>{this.#tt(),this.#dt(),this.#et()},rr=w=>{let S=w;if(!(S.key!=="Escape"||!n.isConnected)){if(dt.handleEscape()||ct.handleEscape()){S.preventDefault(),S.stopPropagation();return}Zt(),S.preventDefault(),S.stopPropagation()}};this.#_=rr,this.document.addEventListener("keydown",rr,!0),Je.addEventListener("click",Zt),Nt.addEventListener("click",Zt),r.addEventListener("pointerdown",w=>{dt.expanded&&!dt.containsEvent(w)&&dt.close(),ct.expanded&&!ct.containsEvent(w)&&ct.close()}),n.addEventListener("click",w=>{w.target===n&&Zt()}),kt.addEventListener("click",()=>{kt.disabled=!0,e.onClearCache().then(()=>{gt("缓存与本地历史已清理。","success")}).catch(w=>{gt(w instanceof Error?w.message:"缓存清理失败","error")}).finally(()=>{kt.disabled=!1})}),Ke.addEventListener("click",()=>{this.#tt(),this.#G=null,e.onReset(),this.#et()}),r.addEventListener("submit",w=>{w.preventDefault();let S=new FormData(r);try{let M=$t({...t,translationProvider:S.get("provider"),translationMode:S.get("mode"),translationTheme:S.get("translationTheme"),translationEnabled:S.get("translationEnabled")==="on",theme:S.get("theme"),titleFontFamily:S.get("titleFontFamily"),titleCustomFontFamily:S.get("titleCustomFontFamily"),fontFamily:S.get("fontFamily"),customFontFamily:S.get("customFontFamily"),fontRenderingEnabled:S.get("fontRenderingEnabled")==="on",fontWeight:S.get("fontWeight"),fontScale:S.get("fontScale"),lineHeight:S.get("lineHeight"),ai:{baseUrl:S.get("aiBaseUrl"),apiKey:S.get("aiApiKey"),model:S.get("aiModel"),prompt:S.get("aiPrompt"),requestsPerMinute:S.get("aiRpm"),tokensPerMinute:S.get("aiTpm")}});e.onSave(M),this.#tt(),this.#G=null,this.#et()}catch(M){Ot("ai"),gt(M instanceof Error?M.message:"设置无效","error")}});for(let w of["input","change"])r.addEventListener(w,S=>{S.target!==C&&gt("有未保存的更改。")});queueMicrotask(()=>m.get(f)?.focus())}destroy(){this.#tt(),this.#et(),this.#t.destroy()}#tt(){let t=this.#_;t&&(this.#_=null,this.document.removeEventListener("keydown",t,!0))}#dt(){let t=this.#G;this.#G=null,t?.()}#et(){let t=this.#K;t&&(this.#K=null,t())}#Ct(t){return t.kind==="comment"?this.#k.ancestors(t.id):t.parentId===this.#k.story.id?[]:Object.freeze([...this.#k.ancestors(t.parentId),t.parentId])}#Ft(t,e){if(t.kind==="missing"){let v=c(this.document,"div","hnr-missing");v.setAttribute("role","treeitem"),v.setAttribute("aria-level",String(t.depth+1)),v.style.setProperty("--hnr-depth",String(Math.min(t.depth,12)));let R=c(this.document,"button","hnr-missing-button",`加载缺失回复 #${t.id}`);return R.type="button",R.dataset.action="load-missing",R.dataset.commentId=String(t.id),v.append(R),v}let n=this.#k.get(t.id),r=c(this.document,"article","hnr-comment");if(r.dataset.commentId=String(t.id),r.setAttribute("role","treeitem"),r.setAttribute("aria-level",String(t.depth+1)),r.setAttribute("aria-expanded",t.hasChildren?String(!t.collapsed):"false"),r.tabIndex=-1,r.style.setProperty("--hnr-depth",String(Math.min(t.depth,12))),r.dataset.depth=String(t.depth),r.dataset.hasChildren=String(t.hasChildren),r.dataset.collapsed=String(t.collapsed),this.#E.has(t.id)&&(r.dataset.newComment="true"),!n)return r;let o=c(this.document,"span","hnr-tree-rails");o.setAttribute("aria-hidden","true");let s=this.#Z[e+1],a=this.#Ct(t),l=s?this.#Ct(s):[],d=[...a,t.id],h=(v,R)=>{let L=c(this.document,"span",`${v} hnr-tree-collapse-hit`);return L.dataset.action="toggle-comment",L.dataset.commentId=String(R),L};for(let v=0;v<Math.min(t.depth,12);v+=1){let R=this.#k.get(d[v]),L=d[v+1],z=L===void 0?-1:R?.childIds.indexOf(L)??-1,x=R?h("hnr-tree-rail",R.id):c(this.document,"span","hnr-tree-rail");x.style.setProperty("--hnr-rail-level",String(v)),x.dataset.continues=String(!!(R&&z>=0&&z<R.childIds.length-1)),x.dataset.currentParent=String(v===t.depth-1),o.append(x)}t.depth>0&&o.append(c(this.document,"span","hnr-tree-elbow")),!t.collapsed&&l[t.depth]===t.id&&o.append(h("hnr-tree-stem",t.id));let m=c(this.document,"div","hnr-comment-head"),p=null,u=c(this.document,"span","hnr-author-marker",(n.author??"?").slice(0,1).toLocaleUpperCase());u.setAttribute("aria-hidden","true");let y=_r(this.document,n.author);y&&u.append(y);let f=c(this.document,"strong","hnr-author",n.author??(n.deleted?"[deleted]":"unknown")),E=c(this.document,"a","hnr-permalink",`#${n.id}`);if(E.href=`https://news.ycombinator.com/item?id=${n.id}`,E.target="_blank",E.rel="noopener noreferrer",m.append(u,f,E),t.hasChildren){let v=t.collapsed?"展开分支":"收起分支";p=c(this.document,"button","hnr-branch-toggle"),p.type="button",p.dataset.action="toggle-comment",p.dataset.commentId=String(n.id),p.dataset.toggleSymbol=t.collapsed?"+":"−",p.setAttribute("aria-label",v);let R=tt(this.document,v,`hnr-tooltip-branch-${n.id}`);p.setAttribute("aria-describedby",R.id),p.append(R),p.addEventListener("click",L=>{L.stopPropagation(),this.#F=n.id,this.#ot(n.id)})}let I=c(this.document,"div","hnr-comment-body"),T=c(this.document,"div","hnr-original-text"),g=this.#Y.get(n.id)?.sanitizedHtml??et(n.html,this.document,this.document.baseURI);T.innerHTML=g||(n.deleted?"[deleted]":"");let A=c(this.document,"div","hnr-translated-text"),H=c(this.document,"div","hnr-bilingual-text"),C=this.#y.get(n.id);C?.html?A.innerHTML=C.html:A.textContent=C?.text??"",C?.bilingualHtml&&(H.innerHTML=C.bilingualHtml),A.hidden=!C||this.#Q!=="translated",H.hidden=!C||this.#Q!=="bilingual",T.hidden=!!C&&this.#Q!=="original",I.append(T,A,H),I.hidden=t.collapsed;let b=c(this.document,"div","hnr-comment-actions");if(b.setAttribute("aria-label",`评论 #${n.id} 操作`),t.hasChildren){let v=t.collapsed?"展开此分支":"收起此分支",R=c(this.document,"button","hnr-comment-action");R.type="button",R.dataset.action="toggle-comment",R.dataset.commentId=String(n.id),R.setAttribute("aria-label",v);let L=tt(this.document,v,`hnr-tooltip-toggle-${n.id}`);R.setAttribute("aria-describedby",L.id),R.append(V(this.document,t.collapsed?zi:qi),L),R.addEventListener("click",z=>{z.stopPropagation(),this.#F=n.id,this.#ot(n.id)}),b.append(R)}for(let[v,R,L]of Ni){let z=Wr(v,R,this.#y.get(n.id)?.complete===!0),x=c(this.document,"button","hnr-comment-action");x.type="button",x.dataset.commentAction=v,x.dataset.commentId=String(n.id),x.setAttribute("aria-label",z);let O=tt(this.document,z,`hnr-tooltip-${v}-${n.id}`);x.setAttribute("aria-describedby",O.id),x.append(V(this.document,L),O),b.append(x)}return b.hidden=t.collapsed,r.append(o,m,I,b),p&&r.append(p),n.dead&&(r.dataset.dead="true"),r}#Dt(t){let e=t.composedPath(),n=e.find(h=>h instanceof HTMLElement&&h.classList.contains("hnr-comment")),r=e.find(h=>h instanceof HTMLElement&&h.dataset.action!==void 0);if(r?.dataset.action==="toggle-comment"&&r.dataset.commentId?this.#F=Number.parseInt(r.dataset.commentId,10):n?.dataset.commentId&&(this.#F=Number.parseInt(n.dataset.commentId,10)),r?.dataset.action==="scroll-top"){if(t instanceof MouseEvent&&t.button===0&&(t.ctrlKey||t.metaKey)&&this.#o.dataset.hnrExternal==="true")return;t.preventDefault(),this.#f.scrollTop=0;return}let o=e.find(h=>h instanceof HTMLElement);if(!o)return;let s=o.dataset.command;if(s){this.actions.onCommand(s);return}let a=o.dataset.commentAction;if(a&&o.dataset.commentId){this.actions.onCommentAction(a,Number.parseInt(o.dataset.commentId,10));return}let l=r?.dataset.action??o.dataset.action,d=r??o;if(l==="close"&&this.actions.onClose(),l==="dismiss-new-comments"&&this.#_t(),l==="previous-new-comment"&&this.#mt(-1,!1),l==="next-new-comment"&&this.#mt(1,!1),l==="locate-new-comment"&&d.dataset.commentId){let h=Number.parseInt(d.dataset.commentId,10);this.#it(h),this.#ct([h])}if(l==="close-summary"&&this.#B(),l==="locate-summary-comment"&&d.dataset.commentId){let h=Number.parseInt(d.dataset.commentId,10);this.#B(!1),this.#it(h,!0)}l==="toggle-comment"&&d.dataset.commentId&&this.#ot(Number.parseInt(d.dataset.commentId,10)),l==="load-missing"&&d.dataset.commentId&&this.actions.onLoadMissing(Number.parseInt(d.dataset.commentId,10))}#It(t){let e=null;if(t.story.url)try{e=Ee(t.story.url).href}catch{e=null}if(e){this.#o.href=e,this.#o.target="_blank",this.#o.rel="noopener noreferrer",this.#o.dataset.hnrExternal="true",this.#o.setAttribute("aria-label","单击回到评论顶部；Ctrl 或 Command 加鼠标左键打开外链"),this.#i.textContent="单击回顶 · Ctrl + 🖱️左键打开外链";return}this.#o.href="#hnr-reader-comments",this.#o.removeAttribute("target"),this.#o.removeAttribute("rel"),this.#o.removeAttribute("data-hnr-external"),this.#o.setAttribute("aria-label","回到评论顶部"),this.#i.textContent="回到评论顶部"}#At(){let t=this.#R.length;if(t===0){this.#u.hidden=!0;return}this.#x=Math.min(this.#x,t-1);let e=this.#R[this.#x],n=e===void 0?void 0:this.#k.get(e);if(!n||e===void 0){this.#u.hidden=!0;return}let r=n.text.replace(/\s+/g," ").trim(),o=r.length>52?`${r.slice(0,52)}…`:r,s=n.author??"unknown";this.#m.textContent=`新评论 ${this.#x+1}/${t} · ${s}${o?` · ${o}`:""}`,this.#h.dataset.commentId=String(e),this.#h.setAttribute("aria-label",`定位新评论 ${this.#x+1}/${t}，${s}`),this.#u.hidden=!1}#mt(t,e){let n=this.#R.length;if(n===0||(this.#x=(this.#x+t+n)%n,this.#At(),!e))return;let r=this.#R[this.#x];r!==void 0&&(this.#it(r),this.#ct([r]))}#_t(){this.#R.length=0,this.#x=0,this.#u.hidden=!0}#ct(t){let e=this.document.defaultView;if(e)for(let n of t){if(!this.#E.has(n)||this.#P.has(n))continue;let r=e.setTimeout(()=>{this.#P.delete(n),this.#E.delete(n),this.#n.querySelector(`.hnr-comment[data-comment-id="${n}"]`)?.removeAttribute("data-new-comment")},xi);this.#P.set(n,r)}}#$t(t){if(t.key==="Escape"){let o=this.#n.querySelector(".hnr-settings-backdrop");o?(this.#tt(),this.#dt(),o.remove()):this.#L?this.#B():this.actions.onClose(),t.preventDefault();return}if(!this.#u.hidden&&(t.key==="ArrowLeft"||t.key==="ArrowRight")&&!(t.target instanceof HTMLInputElement)&&!(t.target instanceof HTMLTextAreaElement)&&!(t.target instanceof HTMLSelectElement)){this.#mt(t.key==="ArrowLeft"?-1:1,!0),t.preventDefault();return}if(t.key==="Tab"){let o=this.#n.querySelector(".hnr-settings-backdrop");if(!o)return;let s=[...o.querySelectorAll("button:not([disabled]),a[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex='-1'])")].filter(d=>!d.closest("[hidden]")),a=s[0],l=s.at(-1);a&&l&&t.shiftKey&&this.#n.activeElement===a?(l.focus(),t.preventDefault()):a&&l&&!t.shiftKey&&this.#n.activeElement===l&&(a.focus(),t.preventDefault());return}let e=[...this.#n.querySelectorAll(".hnr-comment")],n=this.#n.activeElement?.closest(".hnr-comment");if(!n){this.#n.activeElement===this.#f&&(t.key==="ArrowDown"||t.key==="ArrowUp")&&((t.key==="ArrowDown"?e[0]:e.at(-1))?.focus(),t.preventDefault());return}n.dataset.commentId&&(this.#F=Number.parseInt(n.dataset.commentId,10));let r=e.indexOf(n);if(t.key==="ArrowDown")e[r+1]?.focus();else if(t.key==="ArrowUp")e[r-1]?.focus();else if(t.key==="ArrowLeft"&&n.dataset.commentId){let o=Number.parseInt(n.dataset.commentId,10);if(!this.#V.isCollapsed(o))this.#ot(o);else{let s=this.#k.ancestors(o).at(-1);s&&this.#it(s)}}else if(t.key==="ArrowRight"&&n.dataset.commentId){let o=Number.parseInt(n.dataset.commentId,10);this.#V.isCollapsed(o)&&this.#ot(o)}else return;t.preventDefault()}#ot(t){this.#$+=1;let e=this.#$;this.#T!==null&&this.document.defaultView?.cancelAnimationFrame(this.#T),this.#T=null;let n=`.hnr-comment[data-comment-id="${t}"]`,r=this.#n.querySelector(n)?.getBoundingClientRect(),o=this.#f.getBoundingClientRect(),s=Math.min(44,Math.max(1,r?.height??1)),a=Math.max(o.top,o.bottom-s),l=this.#V.isCollapsed(t),d=Math.min(o.top+Pi,a),h=l?r?.top:r&&r.top>=o.top&&r.top<=a?r.top:d;if(this.actions.onToggleComment(t),h===void 0)return;this.#xt(t,h-o.top),this.#ut(n,t,h);let m=this.document.defaultView;m&&(this.#T=m.requestAnimationFrame(()=>{this.#T=null,!(this.#t.destroyed||e!==this.#$)&&(this.#ut(n,t,h),this.#T=m.requestAnimationFrame(()=>{this.#T=null,!(this.#t.destroyed||e!==this.#$)&&this.#ut(n,t,h)}))}))}#ut(t,e,n){let r=this.#n.querySelector(t);if(!r){let s=this.#Z.findIndex(a=>a.id===e);if(s<0)return;this.#p.scrollToIndex(s),r=this.#n.querySelector(t)}if(!r)return;let o=r.getBoundingClientRect().top-n;Math.abs(o)>.5&&(this.#f.scrollTop+=o)}#it(t,e=!1){if(!this.#k.has(t))return!1;this.#V.reveal(t);let n=this.#V.entries(),r=n.findIndex(a=>a.id===t);if(r<0)return!1;this.#Z=n,this.#p.setEntries(n,this.#Y.size>0?n.map(a=>this.#Y.get(a.id)?.estimatedHeight??112):void 0),this.#p.scrollToIndex(r),this.#F=t,this.#xt(t),this.#C+=1;let o=this.#C;this.#H!==null&&this.document.defaultView?.cancelAnimationFrame(this.#H),this.#H=null;let s=a=>{if(this.#t.destroyed||o!==this.#C)return!0;let l=this.#n.querySelector(`.hnr-comment[data-comment-id="${t}"]`);if(!l)return!1;if(a){let d=this.#f.getBoundingClientRect().top,m=l.getBoundingClientRect().top-d;Math.abs(m)>.5&&(this.#f.scrollTop+=m)}return l.focus({preventScroll:!0}),e&&this.#qt(l),!0};return queueMicrotask(()=>{s(!0)||(this.#p.scrollToIndex(r),s(!0));let a=this.document.defaultView;a&&(this.#H=a.requestAnimationFrame(()=>{this.#H=null,!(this.#t.destroyed||o!==this.#C)&&(s(!1)||(this.#p.scrollToIndex(r),s(!0)))}))}),!0}#qt(t){this.#pt();let e=Number.parseInt(t.dataset.commentId??"",10);Number.isSafeInteger(e)&&(this.#I=e,t.removeAttribute("data-locate-flash"),t.offsetWidth,t.dataset.locateFlash="true",this.#O=this.document.defaultView?.setTimeout(()=>{this.#pt()},Mi)??null)}#Rt(){this.#I!==null&&this.#n.querySelector(`.hnr-comment[data-comment-id="${this.#I}"]`)?.setAttribute("data-locate-flash","true")}#pt(){this.#O!==null&&this.document.defaultView?.clearTimeout(this.#O),this.#O=null,this.#I!==null&&this.#n.querySelector(`.hnr-comment[data-comment-id="${this.#I}"]`)?.removeAttribute("data-locate-flash"),this.#I=null}#xt(t,e=0){this.#q=t,this.#W=e,this.#Lt(Hi)}#Ht(){this.#q!==null&&(this.#Lt(Li),this.#Mt())}#Lt(t){this.#z!==null&&this.document.defaultView?.clearTimeout(this.#z),this.#z=this.document.defaultView?.setTimeout(()=>{this.#ft()},t)??null}#Mt(){let t=this.#q,e=this.document.defaultView;t===null||!e||this.#N!==null||(this.#N=e.requestAnimationFrame(()=>{if(this.#N=null,this.#t.destroyed||this.#q!==t)return;let n=this.#n.querySelector(`.hnr-comment[data-comment-id="${t}"]`);if(!n){let s=this.#Z.findIndex(a=>a.id===t);if(s<0)return;this.#p.scrollToIndex(s),n=this.#n.querySelector(`.hnr-comment[data-comment-id="${t}"]`)}if(!n)return;let r=this.#f.getBoundingClientRect().top+this.#W,o=n.getBoundingClientRect().top-r;Math.abs(o)>.5&&(this.#f.scrollTop+=o),n.focus({preventScroll:!0}),this.#Rt()}))}#ft(){this.#z!==null&&this.document.defaultView?.clearTimeout(this.#z),this.#z=null,this.#N!==null&&this.document.defaultView?.cancelAnimationFrame(this.#N),this.#N=null,this.#q=null,this.#W=0}};function wt(i){return`${Number((i*100).toFixed(2))}%`}function ut(i,t,e,n){let r=t.style.getPropertyValue(e),o=t.style.getPropertyPriority(e);i.push(()=>{r?t.style.setProperty(e,r,o):t.style.removeProperty(e)}),t.style.setProperty(e,n,"important")}var Ne=class{constructor(t,e,n={}){this.document=t;this.scope=e.child();let r=t.documentElement,o=t.body,s=o.querySelector(":scope > center"),a=t.querySelector("#hnmain"),l=t.querySelector("[data-hnr-topbar]"),d=Math.ceil(l?.getBoundingClientRect().height??0),h=d>0?d:48,m=t.defaultView,p=m?.scrollY??0,u=[],y=r.classList.contains("hnr-reader-embedded-right"),f=zt(n.readerRatio),E=f;r.classList.add("hnr-reader-embedded-right"),ut(u,r,"--hnr-reader-workspace-width",wt(f)),ut(u,r,"--hnr-host-workspace-width",wt(1-f)),ut(u,r,"--hnr-host-topbar-height",`${h}px`),ut(u,r,"overflow-x","hidden"),ut(u,r,"overflow-y","hidden"),ut(u,r,"height","100dvh");for(let[b,v]of[["box-sizing","border-box"],["width",wt(1-f)],["min-width","0"],["max-width",wt(1-f)],["height","100dvh"],["max-height","100dvh"],["overflow-x","hidden"],["overflow-y",s?"hidden":"auto"],["overscroll-behavior","contain"],["scrollbar-gutter",s?"auto":"stable"]])ut(u,o,b,v);if(s){for(let[b,v]of[["box-sizing","border-box"],["position","fixed"],["top","var(--hnr-host-topbar-height)"],["right","auto"],["bottom","0"],["left","0"],["width","var(--hnr-host-workspace-width)"],["min-width","0"],["max-width","var(--hnr-host-workspace-width)"],["height","calc(100dvh - var(--hnr-host-topbar-height))"],["max-height","calc(100dvh - var(--hnr-host-topbar-height))"],["margin-left","0"],["margin-right","0"],["overflow-x","hidden"],["overflow-y","auto"],["overscroll-behavior","contain"],["scrollbar-gutter","stable"]])ut(u,s,b,v);p>0&&(s.scrollTop=p)}else p>0&&(o.scrollTop=p);if(a)for(let[b,v]of[["box-sizing","border-box"],["width","100%"],["min-width","0"],["max-width","100%"],["margin-left","0"],["margin-right","0"]])ut(u,a,b,v);this.#t=t.createElement("style"),this.#t.dataset.hnrWorkspaceStyle="true",this.#t.textContent=`
...
`,t);let s=()=>{if(!t.destroyed)try{let a=new At,l=a.load(),d=new nt(es),h=new Ht(document,t),m=()=>{},p=()=>{},u=()=>{},y=()=>{},f=new Be(document,t,`:host {
...
`,a,{theme:r,pageScheduler:d,translationRuntime:h,onHostNavigate:C=>m(C),onSettingsPreview:C=>o.apply(C),onSettingsChange:C=>{o.apply(C),p()},onActiveStoryChange:C=>{if(u(C),C===null){try{y(a.load().lastActiveStoryId)}catch{}p()}}}),E=new Te(document,t,e,h.service),I=new ee(document,i,(C,b)=>{f.open(C,b)},"",t,l.lastActiveStoryId,(C,b,v)=>E.translateMany(C,b,v),(C,b,v)=>E.translateComments(C,b,v));u=C=>I.setActiveStory(C),y=C=>I.setLastReadStory(C),I.install(),p=()=>I.refreshPageProjection();let T=new oe(document,d),g=new ie(document,T,I,t);g.install();let A=new re(document,d),H=new ne(document,A,I,()=>{g.resetForListPage(),f.syncHostDocumentTitle()},t,()=>g.suspendForHostNavigation(),C=>{f.openItem(C)});m=C=>{H.navigate(C)},H.install(),i.kind==="item"&&f.openItem(i.itemId).then(()=>H.showHostPage("/news"))}finally{Ze(document,t)}};document.readyState==="loading"?t.listen(document,"DOMContentLoaded",s,{once:!0}):s(),t.listen(window,"pagehide",a=>{ns(document,t,a)})}location.hostname==="news.ycombinator.com"&&oo();})();
