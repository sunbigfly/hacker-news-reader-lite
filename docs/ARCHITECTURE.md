...
`ReaderWorkspace` 在不超过 900 CSS px 的视口切为全屏 Reader，宿主页保持完整宽度并在浮层显示时设为 inert；关闭时恢复原属性、样式和滚动位置。媒体查询监听器只在工作区生命周期内存在，恢复宽屏时重新应用保存的分栏比例，不因响应式切换写入设置。触控软键盘导致 visual viewport 改变时，窗口尺寸更新合并到一帧；双指缩放时保留浏览器视口行为。
