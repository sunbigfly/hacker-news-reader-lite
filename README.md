...
[![Version](https://img.shields.io/badge/version-0.1.7-f97316?style=flat-square)](https://github.com/sunbigfly/hacker-news-reader-lite/releases)
...
支持 Chrome / Edge + Tampermonkey，以及 Android X 浏览器的内置脚本管理器。
...
手机打开阅读器后，浏览器返回会关闭阅读器并保留宿主页面；背景滚动位置在阅读期间锁定，关闭后恢复。每个分支展开时独立记住按钮的位置，首条新回复保持在该位置，收起时展开按钮恢复到原来的高度；嵌套分支反复展开、收起也保留各自的位置记录。
